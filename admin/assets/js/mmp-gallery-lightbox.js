/**
 * MMP Gallery — vanilla JS lightbox.
 *
 * No dependencies. No jQuery. No external libraries.
 *
 * Activates on any element with class `.mmp-gallery__item--lightbox` that has:
...
(function () {
    ...
    function onBodyClick(e) {
        var item = e.target.closest('.mmp-gallery__item--lightbox');
        if (!item) return;
        e.preventDefault();

        var galleryId = item.getAttribute('data-gallery') || 'mmp-gallery';

        // Collect all items in the same gallery group on the page.
        var allItems = Array.prototype.slice.call(
            document.querySelectorAll('.mmp-gallery__item--lightbox[data-gallery="' + galleryId + '"]')
        );
