/**
 * MMP Document Library — frontend JS (S41)
 *
 * Provides AJAX-driven subfolder navigation and pagination for the
 * [mmp_documents] shortcode without full page reloads.
 *
 * Works with vanilla JS only — no React, no jQuery dependency.
 * Each `.mmp-doc-library` element on the page is initialised independently,
...
(function () {
    ...
    function initLibrary(el) {
        ...
        function render(data, pagination) {
            // Breadcrumb
            var nav = el.querySelector('.mmp-doclib__breadcrumb');
            if (nav) {
                nav.innerHTML = buildBreadcrumb(data.breadcrumb || []);
            }

            // Body: subfolders + table + pagination
            var body = el.querySelector('.mmp-doclib__body');
            if (body) {
                var html = '';
        ...
        function setLoading(on) {
            ...
            if (body) {
                if (on) {
                    body.classList.add('mmp-doclib__body--loading');
                } else {
                    body.classList.remove('mmp-doclib__body--loading');
                }
            }
        }
        ...
        function buildBreadcrumb(crumbs) {
            ...
            var html = '<ol class="mmp-doclib__crumb-list">';
            crumbs.forEach(function (crumb, i) {
                if (i === crumbs.length - 1) {
                    html += '<li class="mmp-doclib__crumb mmp-doclib__crumb--current" aria-current="page">'
                        + esc(crumb.name) + '</li>';
                } else {
                    html += '<li class="mmp-doclib__crumb">'
                        + '<button type="button" class="mmp-doclib__crumb-btn" data-folder-id="' + esc(crumb.id) + '">'
        ...
        function buildSubfolders(subfolders) {
            var html = '<div class="mmp-doclib__subfolders">';
            subfolders.forEach(function (sf) {
                html += '<button type="button" class="mmp-doclib__subfolder" data-folder-id="' + esc(sf.id) + '">'
                    + '<span class="mmp-doclib__subfolder-icon" aria-hidden="true">📁</span>'
                    + '<span class="mmp-doclib__subfolder-name">' + esc(sf.name) + '</span>'
                    + '<span class="mmp-doclib__subfolder-count">' + esc(sf.count) + ' files</span>'
                    + '</button>';
            });
            html += '</div>';
            return html;
        ...
        function buildFileTable(files) {
            if (!files.length) {
                return '<p class="mmp-doclib__empty">No files in this folder.</p>';
            }

            var html = '<div class="mmp-doclib__table-wrap">'
                + '<table class="mmp-doclib__table">'
                + '<thead><tr>'
                + '<th class="mmp-doclib__col-icon" aria-hidden="true"></th>'
                + '<th class="mmp-doclib__col-name">File</th>'
                + '<th class="mmp-doclib__col-size">Size</th>'
                + '<th class="mmp-doclib__col-date">Date</th>'
            ...
            files.forEach(function (file) {
                ...
                html += '<tr class="mmp-doclib__row">'
                    + '<td class="mmp-doclib__col-icon">'
                    +     '<span class="mmp-doclib__icon mmp-doclib__icon--' + cat + '" aria-hidden="true">' + icon + '</span>'
                    + '</td>'
                    + '<td class="mmp-doclib__col-name" data-label="File">'
                    +     '<span class="mmp-doclib__filename" title="' + esc(file.filename) + '">' + esc(file.name) + '</span>'
                    + '</td>'
                    + '<td class="mmp-doclib__col-size" data-label="Size">' + esc(file.size_human) + '</td>'
                    + '<td class="mmp-doclib__col-date" data-label="Date">' + esc(file.date) + '</td>'
                    + '<td class="mmp-doclib__col-dl">'
                    +     '<a href="' + esc(file.url) + '" class="mmp-doclib__download" download '
        ...
        function buildPagination(current, total, found) {
            ...
            var html = '<nav class="mmp-doclib__pagination" aria-label="Page navigation">'
                + '<ul class="mmp-doclib__pages">';

            if (current > 1) {
                html += '<li><button type="button" class="mmp-doclib__page-btn" data-page="' + (current - 1)
                    + '" aria-label="Previous page">&#8592;</button></li>';
            }

            for (var p = 1; p <= total; p++) {
                if (p === current) {
                    html += '<li><span class="mmp-doclib__page-btn mmp-doclib__page-btn--current" aria-current="page">'
                        + p + '</span></li>';
                } else if (p === 1 || p === total || Math.abs(p - current) <= 2) {
                    html += '<li><button type="button" class="mmp-doclib__page-btn" data-page="' + p + '">'
                        + p + '</button></li>';
            ...
            html += '</ul>'
                + '<p class="mmp-doclib__total">' + found + ' files total</p>'
                + '</nav>';
            return html;
        }
    ...
    function boot() {
        document.querySelectorAll('.mmp-doc-library').forEach(initLibrary);
    }
