/**
 * MMP Media Modal Bridge
 *
 * Injects the MMP folder sidebar into any wp.media frame opened by a
 ...
 *   3. On frame `open`, inject #mmp-modal-sidebar into the modal DOM and
 *      dispatch mmp:modal-open so the React app can mount its folder tree.
 *   4. On frame `close`, dispatch mmp:modal-close.
 *   5. Pass mmp_folder_id back to the frame's media query so the grid
...
(function () {
    ...
    // Guard: bail if MmpConfig wasn't injected (not an MMP-enabled page).
    if ( typeof window.MmpConfig === 'undefined' ) {
        return;
    }

    var SIDEBAR_ID   = 'mmp-modal-sidebar';
    ...
    function onFrameOpen( frame ) {
        ...
        window.dispatchEvent( new CustomEvent( 'mmp:modal-open', {
            detail: { frame: frame }
        } ) );
    }
    ...
    function onFrameClose( frame ) {
        ...
        window.dispatchEvent( new CustomEvent( 'mmp:modal-close', {
            detail: { frame: frame }
        } ) );
    ...
    function injectSidebar( frame ) {
        ...
        sidebar.className = 'mmp-modal-sidebar';
        ...
        var inner = contentArea.querySelector( '.mmp-modal-content-inner' );
        if ( ! inner ) {
            inner           = document.createElement( 'div' );
            inner.className = 'mmp-modal-content-inner';
        ...
        window.dispatchEvent( new CustomEvent( 'mmp:mount-modal-sidebar', {
            detail: {
                containerId:  SIDEBAR_ID,
                initialTree:  ( window.MmpConfig && window.MmpConfig.initialTree ) || [],
        ...
        try {
            ...
            if ( library && library.props ) {
                if ( folderId === -1 ) {
                    ...
                    library.props.set( 'mmp_folder_id', folderId );
                }

                // Backbone mirror: set on the query object's props too.
                if ( library.mirroring && library.mirroring.props ) {
                    if ( folderId === -1 ) {
                        library.mirroring.props.unset( 'mmp_folder_id' );
                    } else {
                        library.mirroring.props.set( 'mmp_folder_id', folderId );
                    }
                }
            ...
            window.dispatchEvent( new CustomEvent( 'mmp:folder-changed', {
    ...
    /**
     * Patches a frame's library props to include mmp_folder_id in AJAX queries.
    ...
    function patchFrameQuery( frame ) {
        try {
            ...
            // Override toJSON on the query's props so mmp_folder_id is included
            ...
            if ( originalProps && ! originalProps._mmpPatched ) {
                ...
                originalProps.toJSON = function () {
                    var json = origToJSON();
                    if ( activeFolderId !== -1 ) {
                        json.mmp_folder_id = activeFolderId;
                    }
                    return json;
                };
