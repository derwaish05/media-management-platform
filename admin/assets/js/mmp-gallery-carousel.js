/**
 * MMP Gallery — carousel driver.
 *
 * No dependencies. No jQuery. No external libraries.
 *
 * Activates on every element matching [data-mmp-carousel] that contains:
 *   .mmp-carousel__track   — flex row of slide items
 *   .mmp-carousel__btn--prev / --next — navigation buttons
 *   .mmp-carousel__dot     — page-indicator buttons (optional)
 *
 * Behaviour:
 *   - Slides N items per "page" (reads --mmp-columns CSS custom property).
...
(function () {
    ...
    function initCarousel(wrapper) {
        var viewport  = wrapper.querySelector('.mmp-carousel__viewport');
        var track     = wrapper.querySelector('.mmp-carousel__track');
        var btnPrev   = wrapper.querySelector('.mmp-carousel__btn--prev');
        var btnNext   = wrapper.querySelector('.mmp-carousel__btn--next');
        var dots      = wrapper.querySelectorAll('.mmp-carousel__dot');
        ...
        function getColumns() {
            var raw = getComputedStyle(wrapper).getPropertyValue('--mmp-columns');
        ...
        function goTo(index) {
            ...
            dots.forEach(function (dot, i) {
                dot.classList.toggle('mmp-carousel__dot--active', i === pageIndex);
            });
        }
    ...
    function init() {
        var carousels = document.querySelectorAll('[data-mmp-carousel]');
        carousels.forEach(function (wrapper) {
            // Avoid double-init.
            if (wrapper.getAttribute('data-mmp-carousel-init')) return;
            wrapper.setAttribute('data-mmp-carousel-init', '1');
    ...
    window.MmpCarousel = { init: init, initCarousel: initCarousel };
}());
