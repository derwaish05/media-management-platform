/* global wp */
/**
 * MMP Gallery block — editor registration.
 *
...
(function () {
    ...
    // -------------------------------------------------------------------------
    // Helper — flatten the nested MmpFolder tree into SelectControl options
    // -------------------------------------------------------------------------
    ...
    function MmpGalleryEdit(props) {
        ...
        var folderState   = useState([{ label: __('Loading…', 'mmp'), value: 0 }]);
        ...
        var blockProps = useBlockProps({
            className: 'mmp-gallery-editor-preview',
        });
        ...
        var inspector = el(
            ...
            el(
                PanelBody,
                { title: __('Folder', 'mmp'), initialOpen: true },
                loading
                    ? el(Spinner, null)
                    : el(SelectControl, {
                        label: __('Source folder', 'mmp'),
                        value: folderId,
            ...
            el(
                PanelBody,
                { title: __('Layout', 'mmp'), initialOpen: false },
                el(SelectControl, {
                    label: __('Gallery layout', 'mmp'),
                    value: layout,
                    options: [
                        { label: __('Grid',    'mmp'), value: 'grid' },
                        { label: __('Masonry', 'mmp'), value: 'masonry' },
                        { label: __('Flex',    'mmp'), value: 'flex' },
                    ],
                    onChange: function (val) { setAttributes({ layout: val }); },
                    __nextHasNoMarginBottom: true,
                }),
                el(RangeControl, {
                    label: __('Columns', 'mmp'),
                    value: columns,
                    min: 1,
                    max: 8,
                ...
                el(SelectControl, {
                    label: __('Image size', 'mmp'),
                    value: imageSize,
                    options: [
                        { label: __('Thumbnail', 'mmp'), value: 'thumbnail' },
                        { label: __('Medium',    'mmp'), value: 'medium' },
                        { label: __('Large',     'mmp'), value: 'large' },
                        { label: __('Full size', 'mmp'), value: 'full' },
                    ],
            ...
            el(
                PanelBody,
                { title: __('Options', 'mmp'), initialOpen: false },
                el(ToggleControl, {
                    label:   __('Enable lightbox', 'mmp'),
                    help:    __('Wraps each image in a full-size link for lightbox plugins.', 'mmp'),
                    checked: lightbox,
        ...
        var preview = el(
            ...
            el(
                'p',
                { className: 'mmp-gallery-editor-preview__label' },
                folderId > 0
                    ? __('MMP Gallery', 'mmp') + ' — ' + folderName +
                      ' · ' + layout + ' · ' + columns + ' col'
                    : __('MMP Gallery — select a folder in the block settings →', 'mmp')
            )
        );
    ...
    blocks.registerBlockType('mmp/gallery', {
        apiVersion: 3,

        title:       __('MMP Gallery', 'mmp'),
        description: __('Display images from a Media Management Platform folder.', 'mmp'),
        category:    'media',
        icon:        'format-gallery',
        keywords:    [__('gallery', 'mmp'), __('folder', 'mmp'), __('images', 'mmp')],
