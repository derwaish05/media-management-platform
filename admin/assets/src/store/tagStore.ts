 ...
 *  - Expose actions that proxy to the mmp/v1/tags and mmp/v1/files REST routes.
 */

import { create } from 'zustand'
import type { MmpTag } from '@/types'
...
interface TagState {
  ...
  createTag: (name: string, color: string) => Promise<MmpTag>
  updateTag: (id: number, data: Partial<Pick<MmpTag, 'name' | 'color'>>) => Promise<MmpTag>
  deleteTag: (id: number) => Promise<void>

  // File-level tag management
  getFileTags: (attachmentId: number) => Promise<MmpTag[]>
...
export const useTagStore = create<TagState>((set, get) => ({
  ...
  fetchTags: async () => {
    ...
    try {
      const tags = await apiFetch<MmpTag[]>('tags')
  ...
  createTag: async (name, color) => {
    const tag = await apiPost<MmpTag>('tags', { name, color })
    set((s) => ({ tags: [...s.tags, tag] }))
    return tag
  },

  updateTag: async (id, data) => {
    const tag = await apiPut<MmpTag>(`tags/${id}`, data)
  ...
  getFileTags: (attachmentId) => {
    return apiFetch<MmpTag[]>(`files/${attachmentId}/tags`)
  },
