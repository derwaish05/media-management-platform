 ...
 * against the MMP REST API. Clipboard (cut/paste) operations are handled
 * entirely client-side until paste is confirmed.
 */

import { create } from 'zustand'
import type { MmpFolder } from '@/types'
...
interface FolderState {
  // Data
  tree: MmpFolder[]
  flatMap: Record<number, MmpFolder>
  ...
  createFolder: (name: string, parentId?: number) => Promise<MmpFolder>
  ...
  // Helpers
  getFolderById: (id: number) => MmpFolder | undefined
  getAncestors: (id: number) => MmpFolder[]
...
/**
 * Recursively walk the folder tree and produce a flat id → MmpFolder map
 * for O(1) lookups without repeated tree traversals.
 */
function buildFlatMap(tree: MmpFolder[]): Record<number, MmpFolder> {
  const map: Record<number, MmpFolder> = {}

  function walk(folders: MmpFolder[]): void {
 ...
 */
function collectDescendantIds(folderId: number, tree: MmpFolder[]): number[] {
  const ids: number[] = []

  function findAndCollect(folders: MmpFolder[]): boolean {
    for (const folder of folders) {
  ...
  function collectAll(folders: MmpFolder[]): void {
    for (const folder of folders) {
      ids.push(folder.id)
      if (folder.children.length > 0) {
        collectAll(folder.children)
...
const initialTree: MmpFolder[] =
  typeof window !== 'undefined' && window.MmpConfig?.initialTree
    ? window.MmpConfig.initialTree
    : []

// Restore active folder from URL so the sidebar matches what PHP rendered
// on page load. e.g. ?mmp_folder_id=5 → activeFolder 5
//                     ?mmp_folder_id=0 → activeFolder 0 (Uncategorized)
//                     (absent)         → activeFolder null (All Files)
function getInitialActiveFolder(): number | null {
  if (typeof window === 'undefined') return null
  const v = new URLSearchParams(window.location.search).get('mmp_folder_id')
...
export const folderStore = create<FolderState>((set, get) => ({
  ...
  fetchTree: async (viewGlobal?: boolean) => {
    ...
    try {
      ...
      const result = await apiFetch<{ tree: MmpFolder[]; total: number }>(endpoint)
  ...
  setActiveFolder: (id) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (id === null) {
        url.searchParams.delete('mmp_folder_id')
      } else {
        url.searchParams.set('mmp_folder_id', String(id))
      }
  ...
  createFolder: async (name, parentId) => {
    set({ loading: true, error: null })
    try {
      const created = await apiPost<MmpFolder>('folders', {
        name,
  ...
  renameFolder: async (id, name) => {
    set({ loading: true, error: null })
    try {
      await apiPut<MmpFolder>(`folders/${id}`, { name })
      await get().fetchTree()
    } catch (err) {
  ...
  moveFolder: async (id, newParentId) => {
    set({ loading: true, error: null })
    try {
      await apiPost<MmpFolder>(`folders/${id}/move`, { parent_id: newParentId })
      await get().fetchTree()
    } catch (err) {
  ...
  updateColor: async (id, color) => {
    ...
    const patchColor = (nodes: MmpFolder[]): MmpFolder[] =>
      nodes.map((n) =>
        n.id === id
          ? { ...n, color }
    ...
    try {
      await apiPut<MmpFolder>(`folders/${id}`, { color })
    } catch (err) {
