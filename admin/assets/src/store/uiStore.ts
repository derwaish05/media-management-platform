 ...
 * server persists them in wp_mmp_user_prefs. The debounce timeout ID is kept
...
interface UiState {
  ...
  setSortField: (field: MmpSortField) => void
  setSortDir: (dir: MmpSortDir) => void
...
function schedulePrefsSave(patch: Partial<{ sidebar_w: number; sort_files: MmpSortField; sort_dir: MmpSortDir }>): void {
...
const config =
  typeof window !== 'undefined' && window.MmpConfig ? window.MmpConfig : null

const initialSidebarWidth: number = config?.userPrefs?.sidebar_w ?? 300
const initialSortField: MmpSortField = (config?.userPrefs?.sort_files as MmpSortField) ?? 'date'
const initialSortDir: MmpSortDir     = (config?.userPrefs?.sort_dir as MmpSortDir) ?? 'desc'
