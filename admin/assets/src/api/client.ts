/**
 * Typed fetch wrapper for all REST calls to /wp-json/mmp/v1/
 *
 * Reads restUrl and nonce from window.MmpConfig (injected via wp_localize_script).
...
import type { MmpApiResponse } from '@/types'
...
function getConfig() {
  return window.MmpConfig
}

/**
 * Core request function. Generic over the expected `data` payload type T.
 *
 * Unwraps the MmpApiResponse envelope so callers receive T directly.
...
async function request<T>(
  ...
  let json: MmpApiResponse<T>
  try {
    json = (await response.json()) as MmpApiResponse<T>
  } catch {
    throw new Error(`MMP: unexpected non-JSON response (HTTP ${response.status})`)
  }
