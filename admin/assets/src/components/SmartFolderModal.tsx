 ...
 *
 * Opened via the `mmp:open-smart-folder` custom event dispatched by
 * FolderContextMenu → "Smart Folder Rules…".
 *
 * Persistence: rules are stored as JSON in the `mmp_smart_rules` term-meta
 * key on the folder taxonomy term, managed via:
 *   GET    /mmp/v1/folders/{id}/smart-rules
 *   PUT    /mmp/v1/folders/{id}/smart-rules
 *   DELETE /mmp/v1/folders/{id}/smart-rules
 *
...
import type { MmpSmartCondition, MmpSmartRules } from '@/types'
...
interface MmpOpenSmartFolderEvent extends CustomEvent {
  detail: { folderId: number; folderName: string }
}

// ---------------------------------------------------------------------------
// Hook: listen for mmp:open-smart-folder event
// ---------------------------------------------------------------------------
...
function useSmartFolderEvent() {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      const { folderId, folderName } = (e as MmpOpenSmartFolderEvent).detail
      setState({ folderId, folderName })
    }

    window.addEventListener('mmp:open-smart-folder', handler)
    return () => window.removeEventListener('mmp:open-smart-folder', handler)
  }, [])
...
const SmartFolderModal: React.FC = () => {
  ...
  const [rules,   setRules]   = useState<MmpSmartRules>({ mode: 'AND', conditions: [] })
  ...
  useEffect(() => {
    ...
    apiFetch<{ rules: MmpSmartRules | null }>(`folders/${state.folderId}/smart-rules`)
      .then(({ rules: r }) => {
  ...
  const tagConditionIds = rules.conditions
    .filter((c): c is MmpSmartCondition & { tag_id: number } =>
  ...
  const handleTagChange = useCallback(
    (ids: number[]) => {
      const nonTag: MmpSmartCondition[] = rules.conditions.filter((c) => c.type !== 'tag')
      const tagConds: MmpSmartCondition[] = ids.map((id) => ({ type: 'tag', tag_id: id }))
  ...
  const updateCondition = useCallback(
    (index: number, patch: Partial<MmpSmartCondition>) => {
      setRules((r) => {
        const next = [...r.conditions]
        next[index] = { ...next[index], ...patch } as MmpSmartCondition
