 ...
 *
 * Data flow:
 *   Open:  window event 'mmp:open-permissions' with { folderId: number }
 *   Close: component calls hideContextMenu() and clears local state
 *   API:   all CRUD calls go to /mmp/v1/folders/{id}/permissions via apiFetch
 */
...
const PRESETS: { name: MmpPresetName; label: string; desc: string }[] = [
...
const PermissionsModal: React.FC = () => {
  ...
  const [rules, setRules]         = useState<MmpPermissionRule[]>( [] )
  const [loading, setLoading]     = useState( false )
  const [error, setError]         = useState<string | null>( null )

  // Add-rule form state
  const [addEntity, setAddEntity]   = useState<MmpPermissionEntity>( 'role' )
  ...
  useEffect( () => {
    ...
    window.addEventListener( 'mmp:open-permissions', handler )
    return () => window.removeEventListener( 'mmp:open-permissions', handler )
  }, [] )
  ...
  useEffect( () => {
    ...
    setLoading( true )
    apiFetch<MmpPermissionRule[]>( `folders/${folderId}/permissions` )
      .then( ( rules ) => {
        setRules( rules ?? [] )
        setLoading( false )
      } )
  ...
  const handleToggle = useCallback(
    async ( rule: MmpPermissionRule, bit: 'can_read' | 'can_write' | 'can_delete', val: boolean ) => {
  ...
  const handleAddRule = useCallback(
    async ( e: React.FormEvent ) => {
      ...
      try {
        const saved = await apiPut<MmpPermissionRule>( `folders/${folderId}/permissions`, {
          entity:     addEntity,
  ...
  const handleApplyPreset = useCallback(
    async ( presetName: MmpPresetName ) => {
      ...
      try {
        const saved = await apiPost<MmpPermissionRule>(
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="mmp-perm-title"
        className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
          <div>
            <h2 id="mmp-perm-title" className="text-base font-semibold text-slate-800">
              Folder Permissions
            </h2>
            {folderName && (
            ...
            <form onSubmit={handleAddRule} className="space-y-3">
              <div className="flex gap-2 items-end flex-wrap">
                <div>
                  ...
                  <select
                    value={addEntity}
                    onChange={( e ) => setAddEntity( e.target.value as MmpPermissionEntity )}
