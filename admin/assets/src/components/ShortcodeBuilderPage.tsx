 ...
 *
 * Mounted by main.tsx into #mmp-builder-root.
 *
 * 5-step wizard:
...
function flattenFolders(folders: MmpFolder[]): MmpFolder[] {
  const out: MmpFolder[] = []
  const walk = (nodes: MmpFolder[]) => {
...
function buildShortcode(opts: BuilderOptions): string {
  return (
    `[mmp_gallery folder="${opts.folderId}" layout="${opts.layout}" ` +
...
const ShortcodeBuilderPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (w.MmpCarousel?.initCarousel) {
      container.querySelectorAll<HTMLElement>('[data-mmp-carousel]:not([data-mmp-carousel-init])').forEach((el) => {
        w.MmpCarousel.initCarousel(el)
      })
    }
  }, [preview, opts.layout])
  ...
  const loadPreview = useCallback(async () => {
    ...
    try {
      const { restUrl, nonce } = window.MmpConfig
  ...
  return (
    <div className="mmp-builder-page max-w-3xl mx-auto py-8 px-4 font-sans">
      <div className="mb-6">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-slate-500">
                These are the exact attribute values used by the <code>mmp/gallery</code> block.
