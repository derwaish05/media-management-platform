...
const FolderNode: React.FC<FolderNodeProps> = React.memo(
  ({
    ...
    const handleKeyDown = useCallback(
      (e: React.KeyboardEvent<HTMLDivElement>) => {
        switch (e.key) {
          ...
          case 'Delete':     e.preventDefault(); window.dispatchEvent(new CustomEvent('mmp:delete-folder', { detail: { folderId: folder.id } })); break
        }
      },
    ...
    const handleDragStart = useCallback((e: React.DragEvent<HTMLDivElement>) => {
      e.dataTransfer.setData('mmp-folder-id', String(folder.id))
      e.dataTransfer.effectAllowed = 'move'
      window.dispatchEvent(new CustomEvent('mmp:announce', { detail: { message: `Dragging folder ${folder.name}. Drop on a target folder to move it.` } }))
    ...
    const handleDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => {
      ...
      const draggedId = parseInt(e.dataTransfer.getData('mmp-folder-id'), 10)
      if (!isNaN(draggedId) && draggedId !== folder.id) {
        onDrop(draggedId, folder.id)
        window.dispatchEvent(new CustomEvent('mmp:announce', { detail: { message: `Folder moved into ${folder.name}.` } }))
      }
    ...
    const handleDeleteClick = useCallback((e: React.MouseEvent) => {
      e.stopPropagation()
      window.dispatchEvent(new CustomEvent('mmp:delete-folder', { detail: { folderId: folder.id } }))
    }, [folder.id])
    ...
    return (
      ...
      >
        ...
        <div
          ref={rowRef}
          className={`mmp-node-row${isDragOver ? ' mmp-node-row--drag-over' : ''}${isPasteTarget ? ' mmp-paste-target' : ''}`}
          style={{
        ...
        >
          ...
          {!isRenaming && (
            <div className="mmp-node-actions">
              <button
                type="button"
                className="mmp-node-action-btn"
              ...
              <button
                type="button"
                className="mmp-node-action-btn mmp-node-action-btn--danger"
