...
const MenuItem: React.FC<MenuItemProps> = ({
  ...
  const colorClasses =
    variant === 'danger'
      ...
      : 'text-mmp-text hover:bg-slate-100'
...
const FolderContextMenu: React.FC = () => {
  ...
  const handleNewSubfolder = useCallback(() => {
    ...
    window.dispatchEvent(
      new CustomEvent('mmp:new-subfolder', { detail: { folderId } }),
    )
    close()
  }, [folderId, close])
  ...
  const handleRename = useCallback(() => {
    if (folderId == null) return
    window.dispatchEvent(
      new CustomEvent('mmp:rename-folder', { detail: { folderId } }),
    )
    close()
  }, [folderId, close])
  ...
  const handleOpenPermissions = useCallback(() => {
    if (folderId == null) return
    window.dispatchEvent(
      new CustomEvent('mmp:open-permissions', {
  ...
  const handleOpenSmartFolder = useCallback(() => {
    if (folderId == null) return
    window.dispatchEvent(
      new CustomEvent('mmp:open-smart-folder', {
        detail: { folderId, folderName: folder?.name ?? '' },
      }),
    )
    close()
  ...
  const handleDownloadZip = useCallback(() => {
    if (folderId == null) return
    const restUrl = window.MmpConfig?.restUrl?.replace(/\/$/, '') ?? ''
      ...
      {showDeleteConfirm ? (
        ...
        <>
          ...
          <div
            role="menuitem"
            tabIndex={0}
            className="flex items-center gap-2 px-3 py-1.5 text-sm text-mmp-text hover:bg-slate-100 cursor-pointer"
