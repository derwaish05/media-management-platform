 ...
 * FolderSidebar.tsx
 *
 * The primary sidebar panel for the MMP plugin. Injected into the WP Media
...
function countFolders(folders: MmpFolder[]): number {
  let total = 0
...
const Badge = ({ count }: { count: number }) => (
  <span className="mmp-badge">{count > 99 ? '99+' : count}</span>
)
...
const VirtualNode = memo(function VirtualNode({
  ...
  return (
    <div
      ...
      className={`mmp-virtual-row${isActive ? ' mmp-virtual-row--active' : ''}`}
    >
...
export const FolderSidebar: React.FC = () => {
  ...
  const showAdminToggle =
    ...
    window.MmpConfig?.folderMode === 'per_user' &&
    window.MmpConfig?.isAdmin === true
  ...
  const displayFolders = useMemo<MmpFolder[]>(() => {
    if (sortDir === 'none') return filteredTree

    const compareFn = (a: MmpFolder, b: MmpFolder) => {
      const cmp = a.name.localeCompare(b.name, undefined, { sensitivity: 'base' })
      return sortDir === 'za' ? -cmp : cmp
    }

    const sortRecursive = (folders: MmpFolder[]): MmpFolder[] =>
      [...folders]
        .sort(compareFn)
  ...
  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent('mmp:folder-selected', {
        detail: {
  ...
  useEffect(() => {
    ...
    window.addEventListener('mmp:new-subfolder', handleNewSubfolder)
    return () => window.removeEventListener('mmp:new-subfolder', handleNewSubfolder)
  }, [])
  ...
  useEffect(() => {
    ...
    window.addEventListener('mmp:delete-folder', handleDeleteFolder)
    return () => window.removeEventListener('mmp:delete-folder', handleDeleteFolder)
  }, [deleteFolder])
  ...
  useEffect(() => {
    ...
    window.addEventListener('mmp:announce', handler)
    return () => window.removeEventListener('mmp:announce', handler)
  }, [announce])
    ...
    >
      ...
      {!sidebarCollapsed && (
        <div className="flex flex-col flex-1 overflow-hidden" style={{ background: C.white }}>
          ...
          {/* ---------------------------------------------------------------- */}
          {showNewFolderInput && (
            <div className="mmp-new-folder-panel flex-shrink-0">
              <input
                autoFocus
                type="text"
                value={newFolderName}
          ...
          {clipboard?.mode === 'cut' && (() => {
            ...
            return (
              <div className="mmp-cut-banner flex-shrink-0">
