/**
 * MMP React entry point.
 *
 * Mounts the <App /> component into #mmp-root when:
 *   1. window.MmpConfig is present (plugin is active on this page)
 *   2. A #mmp-root element exists in the DOM (injected by the PHP admin class)
 *
...
function mount() {
  // Guard — MmpConfig must be localized by PHP before this script runs
  if (!window.MmpConfig) {
    return
  }

  // Primary mount: media library sidebar + bulk action bar
  const rootEl = document.getElementById('mmp-root')
  if (rootEl) {
  ...
  const dupRootEl = document.getElementById('mmp-duplicates-root')
  if (dupRootEl) {
  ...
  const tplRootEl = document.getElementById('mmp-templates-root')
  if (tplRootEl) {
  ...
  const builderRootEl = document.getElementById('mmp-builder-root')
  if (builderRootEl) {
