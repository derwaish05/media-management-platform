 ...
 */
function filterTree(folders: MmpFolder[], query: string): MmpFolder[] {
  const lower = query.toLowerCase()
  const result: MmpFolder[] = []
 ...
 */
export function useFolderFilter(folders: MmpFolder[], query: string): MmpFolder[] {
  return useMemo(() => {
    if (!query.trim()) {
      return folders
    }
