 ...
 *
 * Boots on `plugins_loaded` / `rest_api_init` inside #mmp-root.
 *
 * Responsibilities:
 *  1. Fetch the initial folder tree on mount.
 *  2. Listen to the `mmp:selection-change` custom event (fired by the
 ...
 *     (#mmp-breadcrumb-root and #mmp-toolbar-root) so they appear in the
...
interface MmpSelectionChangeEvent extends CustomEvent {
  detail: { ids: number[] }
}

// ---------------------------------------------------------------------------
// Hook: resolves a portal element once it exists in the DOM.
// The bridge script creates #mmp-breadcrumb-root and #mmp-toolbar-root
...
export default function App() {
  ...
  const breadcrumbRoot = usePortalElement('mmp-breadcrumb-root')
  const toolbarRoot    = usePortalElement('mmp-toolbar-root')
  ...
  useEffect(() => {
    const handleSelectionChange = (e: Event) => {
      const { ids } = (e as MmpSelectionChangeEvent).detail
    ...
    window.addEventListener('mmp:selection-change', handleSelectionChange)

    return () => {
      window.removeEventListener('mmp:selection-change', handleSelectionChange)
    }
  }, [selectAll, clearSelection])

  // Portal target for the sidebar (positioned into WP's media frame by bridge JS)
  const sidebarPortal = usePortalElement('mmp-sidebar-portal')

  // ---- Render --------------------------------------------------------------

  return (
    <>
      {/* Folder sidebar — portaled into #mmp-sidebar-portal which the bridge JS
      ...
      {/* Floating bulk action bar — fixed position, portaled into body so
          the display:none on #mmp-root does not hide it */}
      ...
      {/* Permissions modal — portaled into body; opened via mmp:open-permissions event */}
      {createPortal(<PermissionsModal />, document.body)}

      {/* Smart Folder modal — portaled into body; opened via mmp:open-smart-folder event */}
      <SmartFolderModal />
    </>
  )
}
