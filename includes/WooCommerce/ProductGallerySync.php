 ...
 *
 * Links a WooCommerce product to an MMP folder so the product gallery
 ...
 *  - A "Sync Now" REST endpoint (`POST mmp/v1/woo/sync/{product_id}`) lets
 *    the meta box button trigger an on-demand sync without a page reload.
 *  - `do_action('mmp_woo_gallery_sync', $productId, $folderId, $attachmentIds)`
...
class ProductGallerySync {
    ...
    private const META_LINKED_FOLDER = '_mmp_linked_folder';
    private const META_BOX_ID        = 'mmp_woo_folder_sync';
    private const REST_NAMESPACE      = 'mmp/v1';
    ...
    public function addMetaBox(): void {
        add_meta_box(
            self::META_BOX_ID,
            __( 'MMP — Sync Gallery from Folder', 'media-management-platform'),
            [ $this, 'renderMetaBox' ],
            'product',
            'side',
            'default'
    ...
    public function renderMetaBox( \WP_Post $post ): void {
        ...
        wp_nonce_field( 'mmp_woo_meta_box', 'mmp_woo_nonce' );
        ?>
        <p>
            <label for="mmp_linked_folder">
                <strong><?php esc_html_e( 'Linked MMP Folder', 'media-management-platform'); ?></strong>
            </label><br>
            <select id="mmp_linked_folder" name="mmp_linked_folder" style="width:100%;margin-top:4px;">
                <option value="0"><?php esc_html_e( '— None —', 'media-management-platform'); ?></option>
        ...
        <p style="font-size:12px;color:#646970;margin-top:4px;">
            <?php esc_html_e( 'Images from the selected folder are added to the product gallery automatically when files change.', 'media-management-platform'); ?>
        </p>
        <p style="margin-top:8px;">
            <button type="button" id="mmp-woo-sync-now" class="button button-secondary"
                    style="width:100%;"
                    data-url="<?php echo esc_url( $restUrl ); ?>"
                    data-nonce="<?php echo esc_attr( $nonce ); ?>">
                <?php esc_html_e( 'Sync Now', 'media-management-platform'); ?>
            </button>
            <span id="mmp-woo-sync-result"
                  style="display:block;margin-top:6px;font-size:12px;min-height:1em;"></span>
        </p>
        <script>
        (function () {
            var btn    = document.getElementById('mmp-woo-sync-now');
            var result = document.getElementById('mmp-woo-sync-result');
            if ( ! btn ) return;

            btn.addEventListener('click', function () {
                btn.disabled       = true;
                result.style.color = '';
                result.textContent = <?php echo wp_json_encode( __( 'Syncing…', 'media-management-platform') ); ?>;
                ...
                .then(function (data) {
                    if ( data && data.success ) {
                        ...
                        result.textContent = ( data.data && data.data.message )
                            ? data.data.message
                            : <?php echo wp_json_encode( __( 'Synced!', 'media-management-platform') ); ?>;
                    } else {
                        result.style.color = '#cc1818';
                        result.textContent = ( data && data.message )
                            ? data.message
                            : <?php echo wp_json_encode( __( 'Sync failed.', 'media-management-platform') ); ?>;
                    }
                    btn.disabled = false;
                })
                .catch(function () {
                    result.style.color = '#cc1818';
                    result.textContent = <?php echo wp_json_encode( __( 'Request failed. Please try again.', 'media-management-platform') ); ?>;
                    btn.disabled = false;
                });
            });
        })();
    ...
    public function saveMetaBox( int $postId ): void {
        // Verify nonce.
        if ( empty( $_POST['mmp_woo_nonce'] )
            || ! wp_verify_nonce(
                sanitize_text_field( wp_unslash( $_POST['mmp_woo_nonce'] ) ),
                'mmp_woo_meta_box'
            )
        ) {
            return;
        ...
        $folderId = absint( $_POST['mmp_linked_folder'] ?? 0 );
    ...
    /**
     * POST mmp/v1/woo/sync/{product_id}
     *
    ...
    public function restSyncNow( \WP_REST_Request $request ): \WP_REST_Response {
        ...
        if ( $folderId <= 0 ) {
            return new \WP_REST_Response(
                [
                    'success' => false,
                    'message' => __( 'No MMP folder linked to this product.', 'media-management-platform'),
                ],
                422
            );
        }
            ...
            [
                ...
                'data'    => [
                    'message' => sprintf(
                        ...
                        _n( '%d image synced.', '%d images synced.', $count, 'media-management-platform'),
                        $count
                    ),
                    'count'   => $count,
                ],
     ...
     *
     * @param int $folderId  MMP folder term ID.
     */
    ...
    /**
     * Sync the WooCommerce product gallery from the linked MMP folder.
     *
     ...
     * WooCommerce standard), then fires `mmp_woo_gallery_sync`.
     *
    ...
    public function syncGallery( int $productId ): int {
        ...
        /**
         * Fires after MMP syncs a WooCommerce product gallery from a folder.
         *
         * @since 1.0.0
         *
         * @param int   $productId      WooCommerce product post ID.
         * @param int   $folderId       MMP folder term ID that was synced.
    ...
    /**
     * Return IDs of all products whose `_mmp_linked_folder` equals $folderId.
     *
     * @param  int $folderId
     * @return int[]
     */
    ...
    /**
     * Resolve a term-taxonomy ID to an MMP folder term ID.
     *
