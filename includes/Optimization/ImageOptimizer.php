 ...
 *   _mmp_webp_path      — absolute path to the WebP sidecar (if created)
 *   _mmp_avif_path      — absolute path to the AVIF sidecar (if created)
 *   _mmp_opt_savings    — bytes saved vs. the original (int, can be 0 or negative)
 *   _mmp_opt_format     — 'webp' | 'avif' | 'skipped' | 'error'
 *
...
class ImageOptimizer {
    ...
    public const OPTION_NAME   = 'mmp_optimization_settings';
    public const CRON_HOOK     = 'mmp_optimize_attachment';

    public const META_WEBP     = '_mmp_webp_path';
    public const META_AVIF     = '_mmp_avif_path';
    public const META_SAVINGS  = '_mmp_opt_savings';
    public const META_FORMAT   = '_mmp_opt_format';
    ...
    public function getStats(): array {
        ...
        $total = (int) $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            "SELECT COUNT(DISTINCT post_id) FROM {$wpdb->postmeta} WHERE meta_key = '_mmp_opt_format'"
        );
