 ...
 *  - Evaluate Smart Folder rules stored as term-meta (`mmp_smart_rules`)
...
class TagService {
    ...
    /**
     * Narrow down the AJAX backbone query when `mmp_tags[]` is in the request.
     *
    ...
    public function evaluateSmartFolder( int $folderId ): array {
        $rulesJson = get_term_meta( $folderId, 'mmp_smart_rules', true );
    ...
    public function isSmart( int $folderId ): bool {
        $raw = get_term_meta( $folderId, 'mmp_smart_rules', true );
    ...
    private function getModeFromRequest( ?array $source = null ): string {
        ...
        return isset( $source['mmp_tag_mode'] ) && $source['mmp_tag_mode'] === 'AND' ? 'AND' : 'OR';
    }
}
