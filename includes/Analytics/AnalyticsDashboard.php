...
/**
 * Drives the analytics subsystem for Media Management Platform.
 *
 * Responsibilities:
 ...
 *    flushed into wp_mmp_analytics in a single INSERT on shutdown (or when the
...
class AnalyticsDashboard {
    ...
    private const QUEUE_OPTION = 'mmp_analytics_queue';
    ...
    public function register(): void {
        ...
        add_action( 'admin_footer', [ $this, 'injectTrackingScript' ] );
    ...
    public function enqueueAssets( string $hookSuffix ): void {
        if ( 'media_page_mmp-analytics' !== $hookSuffix ) {
            return;
        }

        wp_enqueue_script(
            'mmp-chartjs',
            MMP_URL . 'admin/assets/vendor/chart.umd.js',
            [],
            '4.4.2',
            true
        );
    ...
    public function registerAdminPage(): void {
        add_media_page(
            __( 'Media Analytics', 'media-management-platform'),
            __( 'Analytics', 'media-management-platform'),
            'manage_options',
            'mmp-analytics',
            [ $this, 'renderPage' ]
        );
    ...
    public function renderPage(): void {
        if ( ! current_user_can( 'manage_options' ) ) {
            wp_die( esc_html__( 'You do not have permission to view this page.', 'media-management-platform') );
        }

        require_once MMP_PATH . 'admin/views/analytics-page.php';
    }
    ...
    public function track( int $attachmentId, string $eventType ): void {
         ...
         * @param string $eventType    One of 'insert', 'download'.
         */
        $allow = apply_filters( 'mmp_analytics_event', true, $attachmentId, $eventType );
    ...
    public function flushQueue(): void {
        ...
        $sql = 'INSERT INTO ' . $wpdb->prefix . 'mmp_analytics'
    ...
    /**
     * Returns storage consumption grouped by MMP folder (taxonomy term).
     *
            ...
            $wpdb->prepare(  // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                "SELECT
                 ...
                 INNER JOIN {$wpdb->term_taxonomy}  AS tt
                     ...
                     AND tt.taxonomy = 'mmp_folder'
                 INNER JOIN {$wpdb->terms} AS t
                     ON t.term_id = tt.term_id
                 INNER JOIN {$wpdb->postmeta} AS pm
                     ON pm.post_id = p.ID
                     AND pm.meta_key = '_mmp_filesize'
    ...
    /**
     * Backfills the `_mmp_filesize` post meta for existing attachments that do
     * not yet have it.
     *
    ...
    public function backfillFilesizes( int $limit = 100 ): array {
        ...
        foreach ( $ids as $id ) {
            ...
            update_post_meta( $id, '_mmp_filesize', $bytes );
        }
            ...
            '' !== $since  // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                ...
                ? $wpdb->prepare( // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                    "SELECT event_type, COUNT(*) AS cnt
                     FROM {$wpdb->prefix}mmp_analytics
                     {$eventWhereClause}
     ...
     * POST to the `mmp/v1/analytics/track` REST endpoint with keepalive:true.
     *
    ...
    public function injectTrackingScript(): void {
        ...
        $restUrl = wp_json_encode( rest_url( 'mmp/v1/analytics/track' ) );
        ...
        ( function() {
            ...
            /**
             * Sends an analytics event to the MMP REST endpoint.
             *
            ...
            document.addEventListener( 'DOMContentLoaded', function() {
                // Wait for wp.media to be available before patching.
