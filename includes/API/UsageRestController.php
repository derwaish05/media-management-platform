...
class UsageRestController {
    ...
    public function getUsage( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( 'attachment' !== get_post_type( $attachmentId ) ) {
            return $this->error( 'not_found', __( 'Attachment not found.', 'media-management-platform'), 404 );
        }
    ...
    public function triggerScan( WP_REST_Request $request ): WP_REST_Response {
        ...
        return $this->success( [
            ...
            'message'    => sprintf(
                ...
                __( 'Scan complete. %1$d posts scanned, %2$d references indexed.', 'media-management-platform'),
                $stats['scanned'],
                $stats['references']
            ),
        ] );
