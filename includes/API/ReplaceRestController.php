...
class ReplaceRestController {
    ...
    public function replace( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( ! wp_verify_nonce( $nonce, 'mmp_replace_' . $id ) && ! wp_verify_nonce( $nonce, 'wp_rest' ) ) {
            return new WP_REST_Response(
                [ 'success' => false, 'message' => __( 'Invalid nonce.', 'media-management-platform') ],
                403
            );
        }
        ...
        if ( empty( $files['file'] ) || $files['file']['error'] !== UPLOAD_ERR_OK ) {
            ...
            return new WP_REST_Response(
                ...
                [ 'success' => false, 'message' => sprintf( __( 'Upload error (code %d).', 'media-management-platform'), $errorCode ) ],
                400
            );
        }
