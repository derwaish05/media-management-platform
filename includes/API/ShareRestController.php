...
class ShareRestController {
    ...
    private const CAP       = 'manage_mmp_folders';
    ...
    public function create( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( null === $this->folderRepo->getById( $folderId ) ) {
            return new WP_REST_Response(
                [ 'success' => false, 'message' => __( 'Folder not found.', 'media-management-platform') ],
                404
            );
        }
        ...
        if ( null === $link ) {
            return new WP_REST_Response(
                [ 'success' => false, 'message' => __( 'Failed to create share link.', 'media-management-platform') ],
                500
            );
        }
    ...
    public function revoke( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( ! $deleted ) {
            return new WP_REST_Response(
                [ 'success' => false, 'message' => __( 'Share link not found.', 'media-management-platform') ],
                404
            );
        }
    ...
    public function permManageSettings(): bool {
        return current_user_can( 'manage_mmp_settings' );
    }
