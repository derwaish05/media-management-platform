...
use MMP\Analytics\AnalyticsDashboard;
use WP_REST_Request;
use WP_REST_Response;

/**
 * Registers and handles REST API routes for the MMP analytics subsystem.
 *
 * Routes registered under namespace `mmp/v1`:
 *
...
class AnalyticsRestController {
    ...
    /** @var string REST namespace shared by all MMP routes. */
     ...
     * POST /analytics/backfill-sizes
     *
     * Populates the `_mmp_filesize` meta for a batch of existing attachments
    ...
    public function export( WP_REST_Request $request ): void {
        ...
        $filename = 'mmp-analytics-' . gmdate( 'Y-m-d' ) . '.csv';
