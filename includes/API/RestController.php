...
use MMP\Folder\ZipService;
use MMP\Media\BatchMetaService;
use MMP\Media\MediaService;
...
/**
 * Registers and handles all REST API routes under /wp-json/mmp/v1/.
 *
 * Namespace  : mmp/v1
 * Base URL   : /wp-json/mmp/v1/
 *
 * All endpoints:
 ...
 *  - Apply filter `mmp_rest_folder_response` on every folder data response.
 *
 * @package MMP\API
 * @since   1.0.0
 */
class RestController {
    ...
    private const NAMESPACE = 'mmp/v1';
    ...
    public function getFolders( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( 'global' === $folderMode ) {
            ...
            if ( ! current_user_can( 'manage_options' ) ) {
                return $this->error( 'rest_forbidden', __( 'You do not have permission to view another user\'s folders.', 'media-management-platform'), 403 );
            }
        ...
        $response->header( 'X-MMP-Total', (string) $count );
    ...
    public function createFolder( WP_REST_Request $request ): WP_REST_Response {
        ...
        try {
            ...
            return $this->error( 'mmp_invalid_argument', $e->getMessage(), 400 );
        } catch ( \RuntimeException $e ) {
            return $this->error( 'mmp_server_error', $e->getMessage(), 500 );
        }
    ...
    public function getFolder( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( null === $folder ) {
            return $this->error( 'mmp_folder_not_found', __( 'Folder not found.', 'media-management-platform'), 404 );
        }
    ...
    public function updateFolder( WP_REST_Request $request ): WP_REST_Response {
        ...
        try {
            ...
            return $this->error( 'mmp_invalid_argument', $e->getMessage(), 422 );
        }
    ...
    public function moveFolder( WP_REST_Request $request ): WP_REST_Response {
        ...
        try {
            ...
            return $this->error( 'mmp_circular_move', $e->getMessage(), 422 );
        }
     ...
     * id == 0 : attachments NOT in any mmp_folder term (Uncategorized).
    ...
    public function downloadZip( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( null === $folder ) {
            return $this->error(
                'mmp_folder_not_found',
                __( 'Folder not found.', 'media-management-platform'),
                404
            );
        }
        ...
        return $this->error( 'mmp_zip_error', __( 'Unexpected error.', 'media-management-platform'), 500 );
    ...
    public function downloadFilesZip( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( empty( $rawIds ) ) {
            return $this->error(
                'mmp_no_files',
                __( 'No attachment IDs provided.', 'media-management-platform'),
                400
            );
        }
        ...
        if ( empty( $attachmentIds ) ) {
            return $this->error(
                'mmp_no_files',
                __( 'No valid attachment IDs provided.', 'media-management-platform'),
                400
            );
        }
        ...
        try {
            ...
            return $this->error( 'mmp_zip_error', $e->getMessage(), 500 );
        }
    ...
    public function assignFile( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( ! $success ) {
            return $this->error(
                'mmp_assign_failed',
                __( 'Failed to assign file to folder.', 'media-management-platform'),
                500
            );
        }
     ...
     * row exists in wp_mmp_user_prefs.
     *
     * @param WP_REST_Request $request
     * @return WP_REST_Response
     */
    public function getUserPrefs( WP_REST_Request $request ): WP_REST_Response {
        ...
        $row = $wpdb->get_row( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $wpdb->prepare(  // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                "SELECT folder_id, sort_files, sort_dir, sidebar_w, ui_theme FROM {$wpdb->prefix}mmp_user_prefs WHERE user_id = %d LIMIT 1",
                $userId
            ),
            ARRAY_A
        );
    ...
    public function updateUserPrefs( WP_REST_Request $request ): WP_REST_Response {
        ...
        if ( null !== $sortFiles ) {
            ...
            if ( in_array( $sortFiles, $validSort, true ) ) {
                ...
                return $this->error( 'mmp_invalid_param', __( 'sort_files must be one of: name, date, modified, author, size.', 'media-management-platform'), 400 );
            }
        }
        ...
        if ( null !== $sortDir ) {
            if ( in_array( $sortDir, [ 'asc', 'desc' ], true ) ) {
                ...
                return $this->error( 'mmp_invalid_param', __( 'sort_dir must be asc or desc.', 'media-management-platform'), 400 );
            }
        }
        ...
        if ( null !== $sidebarW ) {
            ...
            if ( $sidebarW >= 160 && $sidebarW <= 480 ) {
                ...
                return $this->error( 'mmp_invalid_param', __( 'sidebar_w must be between 160 and 480.', 'media-management-platform'), 400 );
            }
        }
        ...
        if ( null !== $uiTheme ) {
            if ( in_array( $uiTheme, [ 'default', 'win11', 'dropbox' ], true ) ) {
                ...
                return $this->error( 'mmp_invalid_param', __( 'ui_theme must be one of: default, win11, dropbox.', 'media-management-platform'), 400 );
            }
        }
    ...
    /**
     * Requires the `manage_mmp_folders` capability (admins + editors by default).
     */
    public function permManageFolders(): bool {
        return current_user_can( 'manage_mmp_folders' );
    }
    ...
    private function success( mixed $data, int $status = 200 ): WP_REST_Response {
        /**
         * Filters the data payload of any successful MMP REST response.
         *
         ...
         */
        $data = apply_filters('mmp_rest_response', $data, $status);
    ...
    /**
     * Returns default user preferences matching the wp_mmp_user_prefs schema.
     *
     * @return array<string, mixed>
     */
    ...
    /**
     * Returns a flat array of all mmp_folder term IDs currently in the database.
     *
