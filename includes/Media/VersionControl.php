 ...
 *   {uploads_base}/mmp-versions/{attachment_id}/v{version_num}/{filename}
 *
...
class VersionControl {
    ...
    private const ARCHIVE_DIR = 'mmp-versions';
     ...
     *
     * Steps:
     *  1. Look up the version row in wp_mmp_versions.
     ...
     *
     * @param  int $versionId  Primary key in wp_mmp_versions.
     * @return array{
     *   attachment_id: int,
    ...
    public function restoreVersion( int $versionId ): array {
        ...
        $table = $wpdb->prefix . 'mmp_versions';
        ...
        if ( ! $row ) {
            throw new \InvalidArgumentException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                ...
                sprintf( __( 'Version record #%d not found.', 'media-management-platform'), $versionId )  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message, not HTML output
            );
        }
        ...
        if ( '' === $archivePath || ! is_readable( $archivePath ) ) {
            throw new \InvalidArgumentException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                ...
                sprintf( __( 'Archived file not found: %s', 'media-management-platform'), $archivePath )  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message, not HTML output
            );
        }
        ...
        if ( '' === $currentPath ) {
            throw new \RuntimeException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                ...
                sprintf( __( 'No file path registered for attachment %d.', 'media-management-platform'), $attachmentId )  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message, not HTML output
            );
        }
        ...
        if ( ! wp_mkdir_p( $targetDir ) ) {
            throw new \RuntimeException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                ...
                sprintf( __( 'Cannot create directory: %s', 'media-management-platform'), $targetDir )  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message, not HTML output
            );
        }
        ...
        if ( ! @copy( $archivePath, $targetPath ) ) {
            throw new \RuntimeException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                ...
                sprintf( __( 'Failed to restore file to: %s', 'media-management-platform'), $targetPath )  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message, not HTML output
            );
        }
        ...
        $newVersionNum = ( (int) get_post_meta( $attachmentId, '_mmp_version_num', true ) ?: 0 ) + 1;
        update_post_meta( $attachmentId, '_mmp_version_num', $newVersionNum );
        ...
        $wpdb->insert( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            ...
            [
                ...
                'notes'         => sprintf(
                    ...
                    __( 'Restored from version #%d', 'media-management-platform'),
                    $versionId
                ),
            ],
            [ '%d', '%d', '%s', '%s', '%s', '%s', '%d', '%s', '%s' ]
    ...
    public function pruneVersions( int $attachmentId, int $keep = self::DEFAULT_KEEP ): int {
         ...
         */
        $keep = (int) apply_filters( 'mmp_version_limit', $keep, $attachmentId );
        $keep  = max( 1, $keep );
        $table = $wpdb->prefix . 'mmp_versions';
