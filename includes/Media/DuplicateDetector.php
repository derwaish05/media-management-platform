 ...
 *      schedules a single WP Cron event (mmp_run_duplicate_scan).
 ...
 *   4. On completion, results are stored in MMP_DUPLICATES_RESULTS_KEY.
...
class DuplicateDetector {
    ...
    public const CRON_HOOK       = 'mmp_run_duplicate_scan';
    public const STATUS_KEY      = 'mmp_duplicate_scan_status';
    public const RESULTS_KEY     = 'mmp_duplicate_results';
    public const PROGRESS_KEY    = 'mmp_duplicate_scan_progress';

    /** Meta key where each attachment's dHash is stored for fast similar-lookups. */
    public const META_DHASH      = '_mmp_dhash';

    /** Cron hook fired after upload to compute + store dHash asynchronously. */
    private const CRON_UPLOAD    = 'mmp_dhash_on_upload';
    ...
    public function register(): void {
        ...
        // Output the JS handler once per admin page footer.
        add_action( 'admin_footer', [ $this, 'printFindSimilarScript' ] );
    }
    ...
    public function runChunk( int $offset = 0 ): void {
        ...
        $accumulator = get_transient( 'mmp_dup_accumulator' );
        ...
        set_transient( 'mmp_dup_accumulator', $accumulator, self::TRANSIENT_TTL );
        ...
        delete_transient( 'mmp_dup_accumulator' );
    }
    ...
    private function buildResults( array $accumulator ): array {
         ...
         */
        $pHashThreshold = max( 0, (int) apply_filters( 'mmp_duplicate_threshold', self::PHASH_THRESHOLD ) );
    ...
    public function processUploadCron( int $id ): void {
        ...
        if ( ! empty( $similar ) ) {
            update_post_meta( $id, '_mmp_similar_ids', $similar );
        } else {
            delete_post_meta( $id, '_mmp_similar_ids' );
        }
    }
     ...
     *
     * Uses the `_mmp_dhash` meta index built by processUploadCron() and
    ...
    public function addSimilarCountToJs( array $data, \WP_Post $post ): array {
        ...
        $similar = get_post_meta( $post->ID, '_mmp_similar_ids', true );
        $data['mmp_similar_count'] = is_array( $similar ) ? count( $similar ) : 0;
    ...
    public function addFindSimilarField( array $fields, \WP_Post $post ): array {
        ...
        $id      = (int) $post->ID;
        $similar = get_post_meta( $id, '_mmp_similar_ids', true );
        $count   = is_array( $similar ) ? count( $similar ) : 0;

        $btnLabel = $count > 0
            ? sprintf( /* translators: %d: number of similar images found */ _n( 'Find Similar (%d found)', 'Find Similar (%d found)', $count, 'media-management-platform'), $count )
            : __( 'Find Similar Images', 'media-management-platform');

        $html = sprintf(
            '<div class="mmp-find-similar-wrap">
                <button type="button"
                        class="button mmp-find-similar-btn"
                        data-id="%d"
                        style="margin-bottom:6px">%s</button>
                <div class="mmp-similar-results" data-id="%d"></div>
            </div>',
            $id,
            esc_html( $btnLabel ),
            $id
        ...
        $fields['mmp_find_similar'] = [
            'label'         => __( 'Visual Duplicates', 'media-management-platform'),
            'input'         => 'html',
            'html'          => $html,
            'show_in_edit'  => true,
            'show_in_modal' => true,
    ...
    public function printFindSimilarScript(): void {
        ...
        if (
            ...
            && ! str_contains( (string) $screen->id, 'media-management-platform')
        ) {
            return;
        }
        ?>
<script id="mmp-find-similar-js">
(function () {
    var REST_BASE = window.mmpConfig && window.mmpConfig.restBase
        ? window.mmpConfig.restBase
        : '<?php echo esc_js( rest_url( 'mmp/v1/' ) ); ?>';
    var NONCE = window.mmpConfig && window.mmpConfig.nonce
        ? window.mmpConfig.nonce
        : '<?php echo esc_js( wp_create_nonce( 'wp_rest' ) ); ?>';

    document.addEventListener('click', function (e) {
        var btn = e.target.closest('.mmp-find-similar-btn');
        if (!btn) return;

        var id        = btn.dataset.id;
        var container = document.querySelector('.mmp-similar-results[data-id="' + id + '"]');
        if (!container) return;

        btn.disabled    = true;
        btn.textContent = '<?php echo esc_js( __( 'Searching…', 'media-management-platform') ); ?>';
        ...
        .then(function (res) {
            if (!res.success || !res.data || !res.data.similar.length) {
                container.innerHTML = '<p style="color:#666;font-size:12px;margin:4px 0"><?php echo esc_js( __( 'No visually similar images found.', 'media-management-platform') ); ?></p>';
                btn.textContent = '<?php echo esc_js( __( 'Find Similar Images', 'media-management-platform') ); ?>';
                btn.disabled    = false;
                return;
            }
            var files = res.data.similar;
            var html  = '<p style="font-size:11px;color:#555;margin:4px 0 6px">' + files.length + ' <?php echo esc_js( __( 'similar image(s):', 'media-management-platform') ); ?></p>';
            ...
            btn.textContent = '<?php echo esc_js( __( 'Refresh', 'media-management-platform') ); ?>';
            btn.disabled    = false;
        })
        .catch(function () {
            container.innerHTML = '<p style="color:#c00;font-size:12px;margin:4px 0"><?php echo esc_js( __( 'Error loading results.', 'media-management-platform') ); ?></p>';
            btn.textContent = '<?php echo esc_js( __( 'Find Similar Images', 'media-management-platform') ); ?>';
            btn.disabled    = false;
        });
    });
