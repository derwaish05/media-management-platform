 ...
 *   $folderId === 0 → Uncategorized (not in any mmp_folder term)
...
class MediaRepository {
    ...
    public function getFilesInFolder(int $folderId, array $args = []): array {
        ...
        if ($folderId > 0) {
            ...
            // Uncategorized — attachments NOT in any mmp_folder term.
            $queryArgs['tax_query'] = [ // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query
                [
                    'taxonomy' => FolderTaxonomy::TAXONOMY,
                    'field'    => 'term_id',
    ...
    /**
     * Returns the mmp_folder term ID an attachment currently belongs to.
    ...
    public function assignToFolder(int $attachmentId, int $folderId): bool {
        if ($folderId === 0) {
            // Clear all mmp_folder term assignments.
