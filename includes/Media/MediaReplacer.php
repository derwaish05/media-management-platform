 ...
 *   - Always         → log the replacement in wp_mmp_versions.
 *
 * Table managed: wp_mmp_versions
 *
 * @package MMP\Media
 * @since   1.0.0
 */
class MediaReplacer {
    ...
    private const META_VERSION = '_mmp_version_num';
    ...
    /**
     * Create the wp_mmp_versions table if it does not exist.
...
";
    ...
    public function addReplaceField( array $fields, \WP_Post $post ): array {
        ...
        $nonce      = wp_create_nonce( 'mmp_replace_' . $post->ID );
        $apiUrl     = esc_attr( rest_url( 'mmp/v1/files/' . $post->ID . '/replace' ) );
        $currentFile = esc_html( basename( (string) get_attached_file( $post->ID ) ) );

        $html = '<div class="mmp-replace-wrap" data-id="' . esc_attr( (string) $post->ID ) . '" data-api="' . $apiUrl . '" data-nonce="' . esc_attr( $nonce ) . '">';
        $html .= '<p class="description">';
        $html .= sprintf(
            /* translators: %s: current filename */
            esc_html__( 'Current file: %s', 'media-management-platform'),
            '<code>' . $currentFile . '</code>'
        );
        $html .= '</p>';
        $html .= '<input type="file" class="mmp-replace-input" accept="*/*" style="margin-top:6px">';
        $html .= '<button type="button" class="button mmp-replace-btn" style="margin-top:6px;margin-left:4px" disabled>';
        $html .= esc_html__( 'Upload &amp; Replace', 'media-management-platform');
        $html .= '</button>';
        $html .= '<span class="mmp-replace-status" style="margin-left:8px;font-size:12px;color:#64748b"></span>';
        $html .= '</div>';

        $fields['mmp_replace'] = [
            'label' => __( 'Replace File', 'media-management-platform'),
            'input' => 'html',
            'html'  => $html,
        ];
        ...
        $versionsUrl = esc_attr( rest_url( 'mmp/v1/files/' . $post->ID . '/versions' ) );

        if ( ! empty( $history ) ) {
            $histHtml  = '<table class="widefat striped" style="margin-top:8px;font-size:12px">';
            $histHtml .= '<thead><tr>';
            $histHtml .= '<th>' . esc_html__( 'Ver.', 'media-management-platform') . '</th>';
            $histHtml .= '<th>' . esc_html__( 'Replaced', 'media-management-platform') . '</th>';
            $histHtml .= '<th>' . esc_html__( 'By', 'media-management-platform') . '</th>';
            $histHtml .= '<th>' . esc_html__( 'Notes', 'media-management-platform') . '</th>';
            $histHtml .= '<th></th>';
            ...
            foreach ( $history as $row ) {
                ...
                $restoreUrl = esc_attr( rest_url( 'mmp/v1/files/' . $post->ID . '/versions/' . $row['id'] . '/restore' ) );
                ...
                if ( '' !== $row['old_file'] && file_exists( $row['old_file'] ) ) {
                    $histHtml .= '<button type="button" class="button button-small mmp-restore-btn"'
                               . ' data-api="' . $restoreUrl . '"'
                               . ' data-nonce="' . esc_attr( $restoreNonce ) . '">'
                               . esc_html__( 'Restore', 'media-management-platform')
                               . '</button>';
                } else {
                    $histHtml .= '<span style="color:#9ca3af">' . esc_html__( 'Archive gone', 'media-management-platform') . '</span>';
                }
            ...
            $fields['mmp_version_history'] = [
                'label' => __( 'Version History', 'media-management-platform'),
                'input' => 'html',
                'html'  => $histHtml,
            ];
        }
    ...
    public function addReplaceData( array $response, \WP_Post $attachment ): array {
        ...
        $response['mmpReplaceUrl']   = rest_url( 'mmp/v1/files/' . $attachment->ID . '/replace' );
        $response['mmpReplaceNonce'] = wp_create_nonce( 'mmp_replace_' . $attachment->ID );
    ...
    public function printReplaceScript(): void {
        ...
        (function () {
            // ── Edit Media page ──────────────────────────────────────────────
            // ── Restore button ───────────────────────────────────────────────
            document.querySelectorAll('.mmp-restore-btn').forEach(function (btn) {
                btn.addEventListener('click', async function () {
                    if (!confirm('<?php echo esc_js( __( 'Restore this version? The current file will be replaced.', 'media-management-platform') ); ?>')) return;
                    const api   = btn.dataset.api;
                    const nonce = btn.dataset.nonce;
                    btn.disabled = true;
                    btn.textContent = '<?php echo esc_js( __( 'Restoring…', 'media-management-platform') ); ?>';
                    try {
                        const res  = await fetch(api, { method: 'POST', headers: { 'X-WP-Nonce': nonce } });
                        const data = await res.json();
                        if (data.success) {
                            btn.textContent = '<?php echo esc_js( __( 'Restored! Reloading…', 'media-management-platform') ); ?>';
                            setTimeout(() => window.location.reload(), 1200);
                        } else {
                            btn.textContent = '<?php echo esc_js( __( 'Error', 'media-management-platform') ); ?>';
                            alert(data.message || '<?php echo esc_js( __( 'Restore failed.', 'media-management-platform') ); ?>');
                            btn.disabled = false;
                        }
                    } catch (e) {
                        btn.textContent = '<?php echo esc_js( __( 'Error', 'media-management-platform') ); ?>';
            ...
            // ── Edit Media page ──────────────────────────────────────────────
            document.querySelectorAll('.mmp-replace-wrap').forEach(function (wrap) {
                const input  = wrap.querySelector('.mmp-replace-input');
                const btn    = wrap.querySelector('.mmp-replace-btn');
                const status = wrap.querySelector('.mmp-replace-status');
                ...
                btn.addEventListener('click', async function () {
                    ...
                    btn.disabled = true;
                    status.textContent = '<?php echo esc_js( __( 'Uploading…', 'media-management-platform') ); ?>';
                    ...
                    try {
                        ...
                        if (data.success) {
                            status.textContent = '<?php echo esc_js( __( 'Replaced! Reloading…', 'media-management-platform') ); ?>';
                            status.style.color = '#16a34a';
                            setTimeout(() => window.location.reload(), 1200);
                        } else {
                            status.textContent = data.message || '<?php echo esc_js( __( 'Error.', 'media-management-platform') ); ?>';
    ...
    public function replace(
         ...
         */
        do_action( 'mmp_before_file_replace', $attachmentId, $oldFilePath, $newFileTmpPath, $newFilename );
        ...
        if ( $this->versionControl->isIdenticalToCurrentFile( $attachmentId, $newFileTmpPath ) ) {
            throw new \UnexpectedValueException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                __( 'The uploaded file is identical to the current version. No replacement was made.', 'media-management-platform')  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message, not HTML output
            );
        }
         ...
         */
        do_action( 'mmp_after_file_replace', $attachmentId, $oldUrl, $newUrl, $filenameChanged );
    ...
    /**
     * Insert a row into wp_mmp_versions.
     */
    ...
    private function versionsTable(): string {
        global $wpdb;
        return $wpdb->prefix . 'mmp_versions';
    }
}
