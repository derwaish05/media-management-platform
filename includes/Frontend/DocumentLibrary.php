 ...
 *
 * Registers [mmp_documents] and renders a browsable, paginated file list for
 * a given MMP folder with support for nested subfolder navigation (AJAX, no
...
class DocumentLibrary {
     ...
     * document-library roots via the [mmp_documents] shortcode. The REST
     * browse endpoint only serves anonymous requests for folders inside
     * one of these roots.
     */
    public const PUBLIC_ROOTS_OPTION = 'mmp_doclib_public_roots';

    private const SHORTCODE   = 'mmp_documents';
    private const STYLE_HANDLE = 'mmp-document-library';
    private const SCRIPT_HANDLE = 'mmp-document-library';
    ...
    /**
     * Process [mmp_documents] and return the initial HTML for the library.
     *
    ...
    public function renderShortcode( array|string $attrs ): string {
        ...
        if ( $folderId <= 0 ) {
            return '<p class="mmp-doclib-notice">'
                . esc_html__( 'Document library: no folder specified.', 'media-management-platform')
                . '</p>';
        }
        ...
        if ( null === $folder ) {
            return '<p class="mmp-doclib-notice">'
                . esc_html__( 'Document library: folder not found.', 'media-management-platform')
                . '</p>';
        }
        ...
        $this->instanceCount++;
        $uid = 'mmp-doclib-' . $this->instanceCount;
        ...
        $restUrl = rest_url( 'mmp/v1/documents/browse' );
        $nonce   = wp_create_nonce( 'wp_rest' );

        ob_start();
        ?>
        <div id="<?php echo esc_attr( $uid ); ?>"
             class="mmp-doc-library"
            ...
            <?php /* ── Breadcrumb ── */ ?>
            <nav class="mmp-doclib__breadcrumb" aria-label="<?php esc_attr_e( 'Folder navigation', 'media-management-platform'); ?>">
    ...
    public function renderBreadcrumb( array $crumbs ): string {
        ...
        $html = '<ol class="mmp-doclib__crumb-list">';
        $last = count( $crumbs ) - 1;

        foreach ( $crumbs as $i => $crumb ) {
            if ( $i === $last ) {
                $html .= sprintf(
                    '<li class="mmp-doclib__crumb mmp-doclib__crumb--current" aria-current="page">%s</li>',
                    esc_html( $crumb['name'] )
                );
            } else {
                $html .= sprintf(
                    '<li class="mmp-doclib__crumb"><button type="button" class="mmp-doclib__crumb-btn" data-folder-id="%d">%s</button></li>',
                    (int) $crumb['id'],
                    esc_html( $crumb['name'] )
                );
            }
    ...
    public function renderSubfolders( array $subfolders ): string {
        ...
        $html = '<div class="mmp-doclib__subfolders">';
        foreach ( $subfolders as $sf ) {
            $count = (int) ( $sf['count'] ?? 0 );
            $html .= sprintf(
                '<button type="button" class="mmp-doclib__subfolder" data-folder-id="%d">'
                . '<span class="mmp-doclib__subfolder-icon" aria-hidden="true">📁</span>'
                . '<span class="mmp-doclib__subfolder-name">%s</span>'
                . '<span class="mmp-doclib__subfolder-count">%s</span>'
                . '</button>',
                (int) ( $sf['id'] ?? 0 ),
                ...
                esc_html(
                    sprintf(
                        ...
                        _n( '%d file', '%d files', $count, 'media-management-platform'),
                        $count
                    )
                )
            );
    ...
    public function renderFileTable( array $files ): string {
        if ( empty( $files ) ) {
            return '<p class="mmp-doclib__empty">'
                . esc_html__( 'No files in this folder.', 'media-management-platform')
                . '</p>';
        }

        $html  = '<div class="mmp-doclib__table-wrap">';
        $html .= '<table class="mmp-doclib__table">';
        $html .= '<thead><tr>'
            . '<th class="mmp-doclib__col-icon" aria-hidden="true"></th>'
            . '<th class="mmp-doclib__col-name">'  . esc_html__( 'File', 'media-management-platform')     . '</th>'
            . '<th class="mmp-doclib__col-size">'  . esc_html__( 'Size', 'media-management-platform')     . '</th>'
            . '<th class="mmp-doclib__col-date">'  . esc_html__( 'Date', 'media-management-platform')     . '</th>'
            . '<th class="mmp-doclib__col-dl"></th>'
            . '</tr></thead>';
        $html .= '<tbody>';
        ...
        foreach ( $files as $file ) {
            ...
            $html .= sprintf(
                '<tr class="mmp-doclib__row">'
                . '<td class="mmp-doclib__col-icon"><span class="mmp-doclib__icon mmp-doclib__icon--%s" aria-hidden="true">%s</span></td>'
                . '<td class="mmp-doclib__col-name" data-label="%s"><span class="mmp-doclib__filename" title="%s">%s</span></td>'
                . '<td class="mmp-doclib__col-size" data-label="%s">%s</td>'
                . '<td class="mmp-doclib__col-date" data-label="%s">%s</td>'
                . '<td class="mmp-doclib__col-dl"><a href="%s" class="mmp-doclib__download" download aria-label="%s">%s</a></td>'
                . '</tr>',
                $category,
                $this->fileIcon( $category ),
                esc_attr__( 'File', 'media-management-platform'),
                $filename,
                $name,
                esc_attr__( 'Size', 'media-management-platform'),
                $size,
                esc_attr__( 'Date', 'media-management-platform'),
                $date,
                $url,
                /* translators: %s: file name */
                esc_attr( sprintf( __( 'Download %s', 'media-management-platform'), $name ) ),
                esc_html__( 'Download', 'media-management-platform')
            );
        }
    ...
    public function renderPagination( int $current, int $total, int $found ): string {
        ...
        $html  = '<nav class="mmp-doclib__pagination" aria-label="' . esc_attr__( 'Page navigation', 'media-management-platform') . '">';
        $html .= '<ul class="mmp-doclib__pages">';

        if ( $current > 1 ) {
            $html .= sprintf(
                '<li><button type="button" class="mmp-doclib__page-btn" data-page="%d" aria-label="%s">&#8592;</button></li>',
                $current - 1,
                esc_attr__( 'Previous page', 'media-management-platform')
            );
        }
        ...
        for ( $p = 1; $p <= $total; $p++ ) {
            if ( $p === $current ) {
                $html .= sprintf(
                    '<li><span class="mmp-doclib__page-btn mmp-doclib__page-btn--current" aria-current="page">%d</span></li>',
                    $p
                );
            } elseif ( $p === 1 || $p === $total || abs( $p - $current ) <= 2 ) {
                $html .= sprintf(
                    '<li><button type="button" class="mmp-doclib__page-btn" data-page="%d">%d</button></li>',
                    $p,
                    $p
                );
            } elseif ( abs( $p - $current ) === 3 ) {
                $html .= '<li><span class="mmp-doclib__page-ellipsis">&hellip;</span></li>';
            }
        }
        ...
        if ( $current < $total ) {
            $html .= sprintf(
                '<li><button type="button" class="mmp-doclib__page-btn" data-page="%d" aria-label="%s">&#8594;</button></li>',
                $current + 1,
                esc_attr__( 'Next page', 'media-management-platform')
            );
        }

        $html .= '</ul>';
        $html .= sprintf(
            '<p class="mmp-doclib__total">%s</p>',
            esc_html(
                sprintf(
                    /* translators: %d: total file count */
                    _n( '%d file total', '%d files total', $found, 'media-management-platform'),
                    $found
                )
            )
        );
    ...
    private function enqueueAssets(): void {
        ...
        wp_enqueue_style(
            self::STYLE_HANDLE,
            MMP_URL . 'admin/assets/css/mmp-document-library.css',
            [],
            MMP_VERSION
        );
        ...
        wp_enqueue_script(
            self::SCRIPT_HANDLE,
            MMP_URL . 'admin/assets/js/mmp-document-library.js',
            [],
            MMP_VERSION,
            true
        );
