...
class ZipService {
    ...
    public function streamFolderZip(int $folderId): never {
        ...
        if (null === $folder) {
            ...
            die(wp_json_encode([
                'success' => false,
                'code'    => 'mmp_folder_not_found',
                'message' => __('Folder not found.', 'media-management-platform'),
            ]));
        }
        ...
        if (!class_exists('ZipArchive')) {
            ...
            die(wp_json_encode([
                'success' => false,
                'code'    => 'mmp_zip_unavailable',
                'message' => __('ZIP extension is not available on this server.', 'media-management-platform'),
            ]));
        }
    ...
    private function buildZip(array $attachmentIds, string $folderName): string {
        $tmpFile = tempnam(sys_get_temp_dir(), 'mmp_zip_');

        if (false === $tmpFile) {
            throw new \RuntimeException('MMP ZipService: could not create temp file.');
        }
        ...
        if (true !== $opened) {
            throw new \RuntimeException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                sprintf('MMP ZipService: ZipArchive::open() failed with code %d.', $opened)  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message, not HTML output
            );
        }

        $zip->setArchiveComment(
            sprintf(
                'Media folder: %s — exported by Media Management Platform',
                $folderName
            )
        );
