...
/**
 * Role-based and user-based permission system for MMP folders.
 *
 ...
 *        canWrite  → manage_mmp_folders
 *        canDelete → manage_mmp_folders
 *
 * Client isolation:
...
class FolderPermission {
    ...
    /**
     * Hook into mmp_folder_tree to prune unreadable folders before they reach
     * any caller (REST, React, etc.).
     */
    public function register(): void {
        add_filter( 'mmp_folder_tree', [ $this, 'filterTree' ], 20, 2 );
        // Clean up permission rows when a folder is deleted.
        add_action( 'mmp_after_folder_delete', [ $this, 'onFolderDelete' ] );
    }
    ...
    // -------------------------------------------------------------------------
    // Tree filter — called via mmp_folder_tree hook
    // -------------------------------------------------------------------------

    /**
     * Recursively removes folders the current user cannot read.
     * Called via the `mmp_folder_tree` filter.
     *
    ...
    public function applyPreset( int $folderId, string $roleName, string $preset ): bool {
        ...
        if ( null === $p ) {
            throw new \InvalidArgumentException( "Unknown MMP permission preset: {$preset}" ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
        }
    ...
    private function check( string $bit, int $folderId, int $userId, int $depth = 0 ): bool {
        ...
        /**
         * Filters the result of an MMP folder permission check.
         *
         * Return a boolean to short-circuit the built-in rule resolution.
         * Return null to let MMP resolve the permission normally.
         *
         ...
         */
        $override = apply_filters( 'mmp_permission_check', null, $bit, $folderId, $userId );
    ...
    private function wpCapFallback( string $bit, int $userId ): bool {
        ...
        return match ( $bit ) {
            ...
            'can_write'  => user_can( $user, 'manage_mmp_folders' ),
            'can_delete' => user_can( $user, 'manage_mmp_folders' ),
            default      => false,
        };
    }
