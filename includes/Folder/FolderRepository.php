...
/**
 * Data access layer for folders (mmp_folder taxonomy terms).
 *
 ...
 * Raw $wpdb usage is reserved for wp_mmp_user_prefs table lookups only.
 *
 * Transient cache key pattern : mmp_tree_{userId} | mmp_tree_global
...
class FolderRepository {
    ...
    private const META_COLOR      = 'mmp_folder_color';
    private const META_USER_ID    = 'mmp_folder_user_id';
    ...
    public function getTree(int $userId = 0): array {
        ...
        // AND folders that have no mmp_folder_user_id meta at all (e.g. folders
    ...
    /**
     * Returns a single folder by term ID, including mmp_folder_color and
     * mmp_folder_user_id meta values.
     *
     * Returns null if the term does not exist or is not an mmp_folder term.
     *
     * @param  int        $termId
    ...
    /**
     * Returns all folders owned by a specific user via mmp_folder_user_id meta.
     *
     * @param  int   $userId
     * @return array<int, array<string, mixed>>
     */
     ...
     *
     * Stores meta: mmp_folder_color (default '#94a3b8'), mmp_folder_user_id.
    ...
    public function create(string $name, int $parentId = 0, int $userId = 0): int {
        ...
        if (is_wp_error($result)) {
            throw new \RuntimeException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                sprintf('MMP FolderRepository::create() failed: %s', $result->get_error_message())  // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- exception message, not HTML output
            );
        }
     ...
     *
     * Pass $termId = 0 to move to Uncategorized (removes all mmp_folder terms).
    ...
    public function assignFile(int $attachmentId, int $termId): bool {
        if (0 === $termId) {
            // Remove all existing mmp_folder term assignments.
    ...
    /**
     * Updates the mmp_folder_color meta for a folder.
     *
     * @param  int    $termId
    ...
    /**
     * Normalises a WP_Term object into the standard MMP folder array shape.
     *
     * @param  \WP_Term         $term
     ...
     * their mmp_folder taxonomy assignment.
     *
    ...
    private function treeTransientKey(int $userId): string {
        return 0 === $userId ? 'mmp_tree_global' : "mmp_tree_{$userId}";
    }
    ...
    private function deleteTreeTransient(int $userId): void {
        ...
        if ($userId === 0) {
            ...
            delete_transient('mmp_tree_global');
            delete_transient("mmp_tree_{$userId}");
        }
    }
}
