...
class FolderService {
     ...
     *   appending " (2)", " (3)", etc.
     * - Fires: do_action('mmp_after_folder_create', $termId, $name, $parentId, $userId)
     *
     * @param  string $name
    ...
    public function createFolder(string $name, int $parentId = 0, int $userId = 0): int {
        ...
        if ('' === $name) {
            throw new \InvalidArgumentException( __( 'Folder name must not be empty.', 'media-management-platform') ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
        }
         ...
         */
        do_action('mmp_before_folder_create', $name, $parentId, $userId);
         ...
         * @param int    $userId   Owner user ID (0 = global).
         */
        do_action('mmp_after_folder_create', $termId, $name, $parentId, $userId);
     ...
     * - Fires: do_action('mmp_after_folder_rename', $termId, $newName)
     *
     * @param  int    $termId
    ...
    public function renameFolder(int $termId, string $newName): bool {
        ...
        if ($success) {
             ...
             */
            do_action('mmp_after_folder_rename', $termId, $newName);
        }
     ...
     * - Fires: do_action('mmp_after_folder_move', $termId, $newParentId)
     *
     * @param  int  $termId
    ...
    public function moveFolder(int $termId, int $newParentId): bool {
        if ($this->isCircularMove($termId, $newParentId)) {
            throw new \InvalidArgumentException( // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
                sprintf(
                    'MMP: Cannot move folder %d into %d — that would create a circular reference.',
        ...
        if ($success) {
             ...
             */
            do_action('mmp_after_folder_move', $termId, $newParentId);
        }
     ...
     * - Fires: do_action('mmp_after_folder_delete', $termId, $recursive)
     *
     * @param  int  $termId
    ...
    public function deleteFolder(int $termId, bool $recursive = false): bool {
         ...
         */
        do_action('mmp_before_folder_delete', $termId, $recursive);
        ...
        if ($success) {
             ...
             */
            do_action('mmp_after_folder_delete', $termId, $recursive);
        }
     ...
     *
     * Applies filter: apply_filters('mmp_folder_tree', $tree, $userId)
     *
    ...
    public function getTree(int $userId = 0): array {
         ...
         */
        return (array) apply_filters('mmp_folder_tree', $tree, get_current_user_id());
    }
     ...
     * Fires: do_action('mmp_after_file_assign', $attachmentId, $termId)
     *
    ...
    public function assignFile(int $attachmentId, int $termId): bool {
         ...
         */
        do_action('mmp_before_file_assign', $attachmentId, $termId);
        ...
        if ($success) {
             ...
             */
            do_action('mmp_after_file_assign', $attachmentId, $termId);
        }
