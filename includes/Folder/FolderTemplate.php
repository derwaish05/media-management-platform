 ...
 * `mmp_folder_templates`. Each record:
 *
 *   {
 *     "id":          int,
 *     "name":        string,
...
class FolderTemplate {
    ...
    private const OPTION_KEY  = 'mmp_folder_templates';
    ...
    public function captureFromFolder( int $folderId, string $name, string $description = '' ): int {
        ...
        if ( '' === $name ) {
            throw new \InvalidArgumentException( __( 'Template name must not be empty.', 'media-management-platform') ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
        }
    ...
    public function importJson( string $json ): int {
        ...
        if ( ! is_array( $data ) ) {
            throw new \InvalidArgumentException( __( 'Invalid JSON payload.', 'media-management-platform') ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
        }

        if ( empty( $data['name'] ) || ! isset( $data['structure'] ) ) {
            throw new \InvalidArgumentException( __( 'JSON must contain "name" and "structure" keys.', 'media-management-platform') ); // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped
        }
    ...
    private function captureSubtree( int $folderId ): array {
        $term = get_term( $folderId, \MMP\Taxonomy\FolderTaxonomy::TAXONOMY );
    ...
    private function buildNodeChildren( int $parentId ): array {
        $children = get_terms( [
            'taxonomy'   => \MMP\Taxonomy\FolderTaxonomy::TAXONOMY,
            'parent'     => $parentId,
            'hide_empty' => false,
            'number'     => 0,
        ] );
        ...
        foreach ( $children as $child ) {
            $color  = (string) get_term_meta( $child->term_id, 'mmp_folder_color', true );
            $nodes[] = [
    ...
    private function createStructure(
        ...
        foreach ( $nodes as $node ) {
            ...
            $result = wp_insert_term(
                $name,
                \MMP\Taxonomy\FolderTaxonomy::TAXONOMY,
                [ 'parent' => $parentId ]
            );

            if ( is_wp_error( $result ) ) {
                // Term with this name may already exist under this parent — fetch it.
                $existing = get_term_by( 'name', $name, \MMP\Taxonomy\FolderTaxonomy::TAXONOMY );
            ...
            if ( '' !== $color ) {
                update_term_meta( $newId, 'mmp_folder_color', $color );
            }
            if ( $userId > 0 ) {
                update_term_meta( $newId, 'mmp_folder_user_id', $userId );
            }
