 ...
 * {uploads}/mmp/{folder-slug}/.
 *
 * Enable via Media > Filesystem settings page or the mmp_filesystem_settings
 * WordPress option.
 *
...
class RealFolderSync {
    ...
    public const OPTION_NAME       = 'mmp_filesystem_settings';
    ...
    public function register(): void {
        add_action( 'mmp_after_folder_create', [ $this, 'onFolderCreate' ], 10, 4 );
        add_action( 'mmp_after_folder_rename', [ $this, 'onFolderRename' ], 10, 2 );
        add_action( 'mmp_after_folder_delete', [ $this, 'onFolderDelete' ], 10, 2 );
        add_action( 'mmp_after_file_assign',   [ $this, 'onFileAssign'   ], 10, 2 );
     ...
     * Use `wp mmp filesystem sync` to reconcile old directories.
     *
    ...
    public function onFolderRename( int $termId, string $newName ): void {
        ...
        // Directory renaming on folder rename requires a full sync.
        // Use `wp mmp filesystem sync` to reconcile.
    ...
    public function captureSlugBeforeDelete( int $termId, string $taxonomy ): void {
        ...
        set_transient( "mmp_deleting_folder_slug_{$termId}", $term->slug, 30 );
    }
    ...
    public function onFolderDelete( int $termId, bool $recursive ): void {
        ...
        $slug = get_transient( "mmp_deleting_folder_slug_{$termId}" );
        delete_transient( "mmp_deleting_folder_slug_{$termId}" );
     ...
     * {uploads}/mmp/ and moves any misplaced files into the correct directory.
     *
    ...
    /**
     * Returns the absolute path to the MMP root directory inside uploads.
     */
    public function getRoot(): string {
        $uploads = wp_get_upload_dir();
        return rtrim( $uploads['basedir'], '/' ) . '/mmp';
    }
    ...
    /**
     * Creates the MMP root and uncategorized directories if they do not exist.
     */
