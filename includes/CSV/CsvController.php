 ...
 *
 * Actions registered:
 *   mmp_export_folders         — streams folder structure CSV (GET link).
 *   mmp_export_assignments     — streams file-assignments CSV (GET link).
 *   mmp_import_folders         — processes uploaded folder-structure CSV (POST form).
 *   mmp_import_assignments     — processes uploaded file-assignments CSV (POST form).
 *
 * All actions require the `manage_mmp_settings` capability and a valid nonce.
 *
 * @package MMP\CSV
 * @since   1.0.0
 */
class CsvController {
    ...
    private const NONCE_EXPORT  = 'mmp_csv_export';
    private const NONCE_IMPORT  = 'mmp_csv_import';
    private const REDIRECT_PAGE = 'upload.php?page=mmp-settings&tab=csv';
    ...
    public function register(): void {
        add_action( 'admin_post_mmp_export_folders',     [ $this, 'handleExportFolders' ] );
        add_action( 'admin_post_mmp_export_assignments', [ $this, 'handleExportAssignments' ] );
        add_action( 'admin_post_mmp_import_folders',     [ $this, 'handleImportFolders' ] );
        add_action( 'admin_post_mmp_import_assignments', [ $this, 'handleImportAssignments' ] );
    }
    ...
    public function handleExportFolders(): void {
        ...
        $this->verifyNonce( 'mmp_export_folders', self::NONCE_EXPORT );
        $this->exporter->streamFolderStructure(); // exits
    }

    public function handleExportAssignments(): void {
        $this->checkPermission();
        $this->verifyNonce( 'mmp_export_assignments', self::NONCE_EXPORT );
    ...
    public function handleImportFolders(): void {
        $this->checkPermission();
        $this->verifyNonce( 'mmp_import_folders', self::NONCE_IMPORT );

        $handle = $this->openUploadedFile( 'mmp_csv_file' );
        if ( null === $handle ) {
            $this->redirectWithNotice( 'error', __( 'No valid CSV file was uploaded.', 'media-management-platform') );
            return;
        }
    ...
    public function handleImportAssignments(): void {
        $this->checkPermission();
        $this->verifyNonce( 'mmp_import_assignments', self::NONCE_IMPORT );
    ...
    private function redirectWithResult( ImportResult $result, string $type ): void {
        ...
        $message = sprintf(
            ...
            __( 'Import complete — %1$d created, %2$d skipped, %3$d errors.', 'media-management-platform'),
            $result->success,
            $result->skipped,
            $result->errors,
        );
