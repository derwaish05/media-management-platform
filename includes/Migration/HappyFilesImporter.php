...
/**
 * Imports folders from the HappyFiles plugin.
 *
 * HappyFiles stores media folders in the `happyfiles_category` taxonomy.
 * Terms follow standard WP parent/child relationships; file assignments
 * live in wp_term_relationships.
 *
 * @package MMP\Migration
 * @since   1.0.0
 */
class HappyFilesImporter implements PluginImporterInterface {

    private const SOURCE_TAXONOMY = 'happyfiles_category';
    private const SLUG            = 'happyfiles';
    ...
    private function importTerms( ImportProgress $progress ): void {
        ...
        foreach ( $terms as $term ) {
            ...
            // HappyFiles stores a hex color in term description — try to extract it.
            $color = '';
            if ( preg_match( '/#[0-9a-fA-F]{3,6}/', (string) $term->description, $m ) ) {
                $color = $m[0];
            }
    ...
    private function saveIdMap( array $map ): void {
        update_option( 'mmp_migration_idmap_' . self::SLUG, $map, false );
    }

    /** @return array<int,int> */
    private function loadIdMap(): array {
        return (array) get_option( 'mmp_migration_idmap_' . self::SLUG, [] );
    }
}
