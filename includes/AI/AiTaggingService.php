 ...
 *
 * Orchestrates AI image analysis, MMP tag creation, and optional
 ...
 *  2. A WP Cron single event `mmp_ai_tag_attachment` is scheduled so the
 *     upload HTTP response is not delayed by the API call.
 *  3. `analyzeAttachment()` sends the image to the configured provider,
 *     creates/finds MMP tags for every label ≥ confidence_threshold, and
 *     optionally auto-assigns the file to the best-matching folder.
 *
 * Attachment meta keys:
 *   _mmp_ai_tags            array  Stored label results (label, confidence, tag_id).
 *   _mmp_ai_folder_suggest  array  Best folder suggestion (folder_id, folder_name, confidence).
 *   _mmp_ai_tagged_at       string ISO 8601 timestamp of last analysis.
 *   _mmp_ai_provider        string Provider used ('aws' | 'google').
 *   _mmp_ai_error           string Last error message; empty string on success.
 *
 * @package MMP\AI
 * @since   1.0.0
 */
class AiTaggingService {
    ...
    private const META_AI_TAGS    = '_mmp_ai_tags';
    private const META_AI_SUGGEST = '_mmp_ai_folder_suggest';
    private const META_AI_AT      = '_mmp_ai_tagged_at';
    private const META_AI_PROV    = '_mmp_ai_provider';
    private const META_AI_ERROR   = '_mmp_ai_error';
    ...
    public function register(): void {
        ...
        add_action( 'mmp_ai_tag_attachment', [ $this, 'analyzeAttachment' ] );
    }
    ...
    public function onUpload( int $attachmentId ): void {
        ...
        wp_schedule_single_event( time(), 'mmp_ai_tag_attachment', [ $attachmentId ] );
    }
     ...
     * create MMP tags, and optionally assign the file to the best matching folder.
     *
    ...
    public function analyzeAttachment( int $attachmentId ): array {
        ...
        if ( $suggestion !== null ) {
             ...
             */
            do_action( 'mmp_ai_folder_suggestion', $attachmentId, $suggestion, $wasAssigned );
        } else {
