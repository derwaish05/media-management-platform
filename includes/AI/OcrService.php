 ...
 *  2. A WP Cron single event `mmp_ocr_attachment` is scheduled so the
 *     upload HTTP response is never delayed by the API call.
 *  3. `runOcr()` calls the configured provider, stores the result in
 *     `mmp_ocr_text` post meta, and records audit meta keys.
 *
 * Attachment meta keys:
 *   mmp_ocr_text     string  Extracted OCR text (public, searchable).
 *   _mmp_ocr_at      string  ISO 8601 timestamp of last OCR run.
 *   _mmp_ocr_error   string  Last error message; empty on success.
 *   _mmp_ocr_provider string Provider that ran the OCR ('aws'|'google').
 *
 * Settings (shared with AI tagging via `mmp_ai_settings` option):
...
class OcrService {
    ...
    public const META_OCR_TEXT  = 'mmp_ocr_text';

    private const META_OCR_AT   = '_mmp_ocr_at';
    private const META_OCR_ERR  = '_mmp_ocr_error';
    private const META_OCR_PROV = '_mmp_ocr_provider';
    ...
    public function register(): void {
        ...
        add_action( 'mmp_ocr_attachment', [ $this, 'runOcr' ] );
    ...
    public function onUpload( int $attachmentId ): void {
        ...
        wp_schedule_single_event( time(), 'mmp_ocr_attachment', [ $attachmentId ] );
    }
    ...
    /**
     * Extend the native WP attachment search to also match `mmp_ocr_text` meta.
     *
    ...
    public function includeOcrInSearch( string $search, \WP_Query $query ): string {
        ...
        $ocrClause = $wpdb->prepare(  // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            " OR EXISTS (SELECT 1 FROM {$wpdb->postmeta} _ocr WHERE _ocr.post_id = {$wpdb->posts}.ID AND _ocr.meta_key = 'mmp_ocr_text' AND _ocr.meta_value LIKE %s)",
            $like
        );
    ...
    public function registerMetaBox(): void {
        add_meta_box(
            'mmp-ocr-text',
            __( 'OCR Extracted Text', 'media-management-platform'),
            [ $this, 'renderMetaBox' ],
            'attachment',
            'normal',
            'low'
    ...
    public function renderMetaBox( \WP_Post $post ): void {
        ...
        if ( '' !== $error ) {
            echo '<span style="color:#d63638;">' . esc_html__( 'OCR error — click to expand', 'media-management-platform') . '</span>';
        } elseif ( '' !== $text ) {
            $wordCount = str_word_count( $text );
            /* translators: %d: number of words extracted by OCR */
            echo esc_html( sprintf( __( 'Extracted text (%d words) — click to expand', 'media-management-platform'), $wordCount ) );
        } else {
            echo esc_html__( 'No OCR text yet — click to expand', 'media-management-platform');
        }
        ...
        if ( '' !== $error ) {
            echo '<p style="color:#d63638;margin:0 0 8px;">'
                . esc_html__( 'Last error:', 'media-management-platform') . ' '
                . esc_html( $error )
                . '</p>';
        }
        ...
        if ( '' !== $text ) {
            ...
            echo '<p style="color:#757575;margin:0;">'
                . esc_html__( 'No text has been extracted yet. OCR runs automatically on upload when a provider is configured under Media › MMP AI Settings.', 'media-management-platform')
                . '</p>';
        }
        ...
        if ( '' !== $at || '' !== $provider ) {
            ...
            if ( '' !== $provider ) {
                ...
                echo esc_html( sprintf( __( 'Provider: %s', 'media-management-platform'), strtoupper( $provider ) ) );
            }
            if ( '' !== $at ) {
                if ( '' !== $provider ) {
                    echo ' &nbsp;|&nbsp; ';
                ...
                echo esc_html( sprintf( __( 'Last run: %s', 'media-management-platform'), $at ) );
            }
            echo '</p>';
        }
