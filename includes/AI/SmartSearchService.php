...
use MMP\Search\AdvancedSearchService;
 ...
 *  2. Query wp_mmp_tags by name (LIKE substring) for any expanded term.
 *  3. Resolve matching attachment IDs from wp_mmp_tag_relationships.
 ...
 * `mmp_ai_search_synonyms` filter, which receives and must return
...
class SmartSearchService {
     ...
     *
     * Checks the built-in synonym table (extended via `mmp_ai_search_synonyms`)
    ...
    public function expandQuery( string $q ): array {
        ...
        $groups = (array) apply_filters( 'mmp_ai_search_synonyms', self::SYNONYMS );
    ...
    /**
     * Find attachment IDs whose MMP tags match any of the given terms
     * (case-insensitive substring match against tag names).
     *
     * Queries the custom tables created in S2:
     *   wp_mmp_tags              — tag name index (includes AI-generated tags)
     *   wp_mmp_tag_relationships — many-to-many pivot (tag_id, attachment_id)
     *
     * @param  string[] $terms
     * @return int[]
     */
    private function findByLabels( array $terms ): array {
        ...
        $tagsTable = $wpdb->prefix . 'mmp_tags';
        $relTable  = $wpdb->prefix . 'mmp_tag_relationships';
