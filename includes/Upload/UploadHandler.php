...
use MMP\Media\MediaRepository;
use MMP\Folder\FolderRepository;

/**
 * Handles all upload-related integration for the Media Management Platform.
 *
 * Responsibilities:
 ...
 *       c. Fall back to the user's last-used folder stored in wp_mmp_user_prefs.
 *  4. Persist the last-used folder per user in wp_mmp_user_prefs.
 *  5. Inject a folder picker <select> + plupload wiring into the WP upload UI.
 *  6. Expose `mmp_folder_id` as an accepted REST API parameter on the
...
class UploadHandler {
     ...
     *   2. Classic upload modal: $_POST['mmp_upload_folder_id'] (set by folder picker).
     *   3. Fallback: user's last-used folder stored in wp_mmp_user_prefs.
     *
    ...
    public function autoAssignFolder( int $attachmentId ): void {
        ...
        if ( $folderId <= 0 && isset( $_POST['mmp_upload_folder_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- folder ID is safe (absint sanitized), nonce check done by WP core upload handler
            $folderId = absint( $_POST['mmp_upload_folder_id'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
            $fromPost = ( $folderId > 0 );
        } elseif ( $folderId > 0 ) {
    ...
    public function injectFolderPickerScript(): void {
        ...
        echo '<div id="mmp-folder-picker-wrap" style="margin:8px 0;">' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- hardcoded HTML with esc_html__() translations
            . '<label for="mmp-upload-folder" style="display:block;font-weight:600;margin-bottom:4px;">'
            . esc_html__( 'Upload to Folder:', 'media-management-platform')
            . '</label>'
            . '<select id="mmp-upload-folder" name="mmp_upload_folder_id" style="max-width:260px;">'
            . '<option value="0">' . esc_html__( '— No Folder (Uncategorized)', 'media-management-platform') . '</option>'
        ...
        // Inline script: wire the <select> value into every plupload upload.
        echo '<script>
(function($){
    $(function(){
        if(typeof wp !== "undefined" && wp.Uploader) {
            ...
            wp.Uploader.prototype.init = function() {
                ...
                this.uploader.bind("BeforeUpload", function(up, file) {
                    var folderSelect = document.getElementById("mmp-upload-folder");
                    if(folderSelect) {
                        up.settings.multipart_params = up.settings.multipart_params || {};
                        up.settings.multipart_params.mmp_upload_folder_id = folderSelect.value;
                    }
                });
            };
        }
    ...
    /**
     * Captures the mmp_folder_id parameter from a REST API attachment upload
    ...
    public function handleRestUploadFolder( \WP_Post $attachment, \WP_REST_Request $request ): \WP_Post {
        $folderId = absint( $request->get_param( 'mmp_folder_id' ) ?? 0 );
    ...
    // -------------------------------------------------------------------------
    // Private — wp_mmp_user_prefs helpers
    // -------------------------------------------------------------------------

    /**
     * Retrieves the last-used folder ID for a user from wp_mmp_user_prefs.
     *
    ...
    private function getUserLastFolder( int $userId ): int {
        ...
        $result = $wpdb->get_var( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $wpdb->prepare(  // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                "SELECT folder_id FROM {$wpdb->prefix}mmp_user_prefs WHERE user_id = %d",
                $userId
            )
        );
    ...
    /**
     * Upserts the last-used folder ID for a user in wp_mmp_user_prefs.
     *
