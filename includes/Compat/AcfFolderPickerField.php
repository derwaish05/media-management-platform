 ...
 *
 * Registers a custom ACF field type "MMP Folder Picker" that renders a
 * hierarchical folder select control. The saved value is the MMP folder
 * term ID (integer).
 *
 * Usage in ACF field group:
 *   Field Type → MMP Folder Picker
...
class AcfFolderPickerField extends \acf_field {
    ...
    public function __construct(
        ...
        $this->name     = 'mmp_folder_picker';
        $this->label    = __( 'MMP Folder Picker', 'media-management-platform');
    ...
    public function render_field_settings( $field ): void {
        acf_render_field_setting( $field, [
            'label'        => __( 'Allow None', 'media-management-platform'),
            'instructions' => __( 'Allow users to select "None" (saves 0).', 'media-management-platform'),
            'type'         => 'true_false',
            'name'         => 'allow_none',
            'ui'           => 1,
        ] );
        ...
        acf_render_field_setting( $field, [
            'label'        => __( 'Return Format', 'media-management-platform'),
            'instructions' => __( 'Specify the value returned by get_field().', 'media-management-platform'),
            'type'         => 'radio',
            'name'         => 'return_format',
            'choices'      => [
                'id'   => __( 'Folder ID (integer)', 'media-management-platform'),
                'name' => __( 'Folder Name (string)', 'media-management-platform'),
            ],
            'layout'       => 'horizontal',
        ] );
    }
    ...
    public function render_field( $field ): void {
        ...
        echo '<select id="' . $id . '" name="' . $name . '" class="acf-mmp-folder-picker">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $id and $name are esc_attr() escaped above
        ...
        if ( $allowNone ) {
            echo '<option value="0"' . selected( $value, 0, false ) . '>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- selected() is a trusted WP function
                . esc_html__( '— None —', 'media-management-platform')
                . '</option>';
        }
