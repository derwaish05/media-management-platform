 ...
 *
 * Responsibilities:
 *  - Registers the 'mmp_folder' taxonomy with WPML so folder terms can be
...
class WpmlIntegration {
    ...
    public function register(): void {
        ...
        // Filter the MMP folder tree by active language.
    ...
    /**
     * Add 'mmp_folder' to the list of taxonomies WPML should translate.
     *
     * @param  string[] $taxonomies
     * @return string[]
     */
    public function registerTaxonomy( array $taxonomies ): array {
        if ( ! in_array( 'mmp_folder', $taxonomies, true ) ) {
            $taxonomies[] = 'mmp_folder';
        }
     ...
     * @param  int                              $viewerId  (unused, passed by mmp_folder_tree filter)
    ...
    private function filterNodes( array $nodes, string $lang ): array {
        ...
        foreach ( $nodes as $node ) {
            ...
            $translatedId = (int) apply_filters( 'wpml_object_id', $termId, 'mmp_folder', false, $lang );
